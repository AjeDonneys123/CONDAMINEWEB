// @signatures: SERVER_BOOT_ID, GlobalInfrastructure, KernelV86_STABLE
...
console.log("------------------------------------------------");
console.log("🚀 KERNEL V86 : FULL API RESTORED + RAW PROXY");
console.log("------------------------------------------------");
...
app.get('/api/system/apply-status', (req, res) => {
    try {
        const statusPath = path.join(process.cwd(), 'apply_status.json');
        if (fs.existsSync(statusPath)) {
            const raw = fs.readFileSync(statusPath, 'utf8');
            if (raw.trim()) return res.json(JSON.parse(raw));
        }
    } catch (e) {}
    res.json({ status: "OK", message: "Kernel V86 Online" });
});

// 2. PROXY BINAIRE (CONSERVÉ CAR IL FONCTIONNE POUR LE SON)
...
// 3. CHARGEMENT TOUS SILOS
const safeLoad = (route, path) => {
    try { app.use(route, require(path)); } catch (e) { console.error("Load Error:", route); }
};
...
mongoose.connect(process.env.MONGODB_URI).then(() => {
    app.listen(port, '0.0.0.0', () => console.log(`🏁 KERNEL V86 READY`));
});
