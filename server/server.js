...
// Injection globale de fetch pour Gemini (Fix Node < 18)
...
// 1. ENREGISTREMENT PRIORITAIRE DES MODÈLES
require('./models/Bug');
...
app.get('/api/deploy-status', async (req, res) => {
    try {
        ...
        // Utilisation d'un chemin relatif sécurisé pour version.json
        const versionPath = path.join(__dirname, 'version.json');
        let versionData = { version: "1.6.0", build: 0 };
...
app.use('/api', require('./features/admin/admin.routes')); // ADMIN centralise /players et /chapters-all
...
app.listen(port, () => console.log(`🚀 SERVEUR REPARTI | PORT ${port}`));