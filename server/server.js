// @signatures: SERVER_BOOT_ID, GlobalInfrastructure, KernelV84_STABLE
...
console.log("------------------------------------------------");
console.log("🚀 KERNEL V84 : MULTIMEDIA FLOW OPTIMIZED");
console.log("------------------------------------------------");

app.use(express.json({ limit: '70mb' }));
app.use(express.urlencoded({ extended: true, limit: '70mb' }));

// 1. ROUTES SYSTÈME (Priorité absolue)
...
app.get('/api/system/apply-status', (req, res) => {
    ...
    res.json({ status: "OK", message: "Kernel V84 Online" });
});

// 2. PROXY BINAIRE (Indispensable pour le décodage Web Audio)
...
app.get(['/api/proxy/:id', '/api/structure/proxy/:id'], async (req, res) => {
    try {
        ...
        // On ne force AUCUN content-type pour laisser le navigateur décoder selon le header Google
