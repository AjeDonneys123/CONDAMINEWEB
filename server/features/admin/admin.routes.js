...
// PATCH /api/teacher/:id/sections (Matières) - FIX HIÉRARCHIE
router.patch('/teacher/:id/sections', async (req, res) => {
    try {
        const { sections, className } = req.body;
        const Teacher = mongoose.model('Teacher');
        
        // 1. On s'assure que les dossiers physiques existent dans /Devoirs/
        if (className && sections.length > 0) {
            const hwRootId = await DriveService.getHomeworkRoot(className);
            for (const s of sections) {
                await DriveService.getOrCreateFolder(normalize(s.name), hwRootId);
            }
        }
        ...
        res.json({
            ...
            drivePath: `CONDACLASSE / JEAN VUILLET / ${className} / Devoirs / ...`,
            message: "Matières synchronisées dans 'Devoirs'"
        });
    } catch (e) { res.status(500).json({ error: e.message }); }
});

// POST /api/chapters - FIX HIÉRARCHIE
router.post('/chapters', async (req, res) => {
    try {
        ...
        const subFolderId = await DriveService.getOrCreateFolder(normalize(subject), hwRootId);
