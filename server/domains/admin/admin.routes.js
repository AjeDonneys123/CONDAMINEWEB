...
/**
 * 🛡️ ROUTES ADMIN V162 - UTF8 FIX & NAME PARSING
 * Fix : Lecture en UTF-8 pour corriger les accents (Ã© -> é).
 * Fix : Suppression du BOM (caractère invisible de début de fichier).
 * Fix : Algorithme de découpage Nom/Prénom plus intelligent.
 */
...
router.post('/import-csv', upload.single('file'), asyncHandler(async (req, res) => {
    ...
    try {
        ...
        console.log(`📂 IMPORT CLASSE: ${className} (NIVEAU: ${level})`);
        ...
        // V162 : Lecture en UTF-8 pour les accents
        let content = fs.readFileSync(req.file.path, 'utf8');
        
        // V162 : Nettoyage du BOM (Byte Order Mark) qui crée le point au début
        ...
        for (let i = 1; i < lines.length; i++) {
            ...
            // V162 : Algorithme de découpage Nom/Prénom Hybride
            let lName = "", fName = "";
            
            if (rawName.includes(',')) {
                // Cas facile : "Nom, Prénom"
                const parts = rawName.split(',');
                lName = parts[0].trim();
                fName = parts[1].trim();
            } else {
                // Cas complexe : "Nom1 Nom2 Prénom1 Prénom2" ou "Nom Prénom"
                const parts = rawName.split(/\s+/); // Découpe par espace
                
                if (parts.length === 2) {
                    // "Pierre Dupont" -> Nom=Pierre, Prénom=Dupont (ou inverse selon le CSV)
                    // Supposons que le CSV est "NOM PRENOM"
                    lName = parts[0];
                    fName = parts[1];
                } else if (parts.length > 2) {
                    // "Arguello Zambrano Amaia Carolina"
                    // On prend les 2 premiers comme Nom de famille (Standard LatAm)
            ...
            // Prénom : Première lettre majuscule, reste minuscule (optionnel, pour faire joli)
            // fName = fName; 
            ...
            const student = await StudentModel.findOneAndUpdate(query,
                { 
                    ...
                    gender: genreStr.includes('Féminin') ? 'F' : 'M'
                },
                { upsert: true, new: true }
            );
...
router.post('/classrooms', asyncHandler(async (req, res) => {
    ...
    let level = req.body.level ? req.body.level.toUpperCase().trim() : guessLevel(name);
    const cls = await mongoose.model('Classroom').findOneAndUpdate(
        { name }, { ...req.body, name, level }, { upsert: true, new: true }
    );
    await StructureDrive.syncBaseStructure();
    res.json(cls);
}));
...
router.post('/students', asyncHandler(async (req, res) => {
    ...
    if (data.classId && !data.currentLevel) {
        const cls = await mongoose.model('Classroom').findById(data.classId);
        if (cls && cls.level) data.currentLevel = cls.level;
    }
    let student;
