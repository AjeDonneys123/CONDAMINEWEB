...
// --- CORRECTION INTELLIGENTE (INCREMENTALE) ---
router.post('/correct/:sessionId', asyncHandler(async (req, res) => {
    ...
    // Mise à jour des instructions (si changées, elles s'appliqueront aux NOUVELLES corrections)
    if (req.body.instructions) {
        session.aiInstructions = req.body.instructions;
    }

    // 1. Création d'une Map des corrections existantes pour éviter les doublons
    ...
    // 2. Parcours de toutes les URLs de copies présentes dans la session
    for (const copyUrl of session.copyUrls) {
        // Est-ce que cette copie a déjà été corrigée ?
        if (existingCorrectionsMap.has(copyUrl)) {
            // OUI : On garde l'ancienne correction (Économie IA & Temps)
            // console.log(`⏩ [SCAN] Skip correction existante pour ${copyUrl}`);
            finalResults.push(existingCorrectionsMap.get(copyUrl));
        } else {
            // NON : C'est une nouvelle copie, on lance l'IA
            try {
                console.log(`🤖 [SCAN] Correction IA lancée pour ${copyUrl}`);
                ...
                console.error("❌ Erreur correction copie:", e);
                // On ajoute une entrée d'erreur pour ne pas perdre la trace
                finalResults.push({
                    originalUrl: copyUrl,
                    studentName: "Erreur Analyse",
                    grade: "?",
                    appreciation: "L'IA n'a pas pu traiter cette image. Vérifiez qu'elle est lisible.",
    ...
    console.log(`✅ [SCAN] Fin de traitement. ${newCorrectionsCount} nouvelles corrections.`);
    res.json(session);
}));
