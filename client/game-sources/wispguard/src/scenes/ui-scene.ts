...
export class UiScene extends Phaser.Scene {
  ...
  public async updateHealthInHud(data: PlayerHealthUpdated): Promise<void> {
    if (data.type === PLAYER_HEALTH_UPDATE_TYPE.INCREASE) {
      const fullHearts = Math.floor(data.currentHealth / 2);
      const hasHalfHeart = data.currentHealth % 2 === 1;
      const totalHearts = Math.floor(DataManager.instance.data.maxHealth / 2);
      this.#hearts.forEach((heart, index) => {
        heart.stop();
        let frame: string = HEART_TEXTURE_FRAME.NONE;
        if (index < fullHearts) frame = HEART_TEXTURE_FRAME.FULL;
        else if (hasHalfHeart && index === fullHearts) frame = HEART_TEXTURE_FRAME.HALF;
        else if (index < totalHearts) frame = HEART_TEXTURE_FRAME.EMPTY;
