  ...
  return (
    <div className="fixed inset-0 z-[6000] bg-slate-50 flex flex-col animate-in zoom-in">
            ...
            <div className="max-w-4xl mx-auto space-y-10">
                ...
                {genMode === 'manual' ? (
                    <div className="p-10 bg-white rounded-[50px] border shadow-sm">
                        <input className="w-full p-4 mb-6 border-b-2 font-black text-xl outline-none" placeholder="Quelle est la question ?" value={currentQ.q} onChange={e=>setCurrentQ({...currentQ, q:e.target.value})} />
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            {currentQ.options.map((opt, i) => (
                                <div key={i} className={`flex items-center gap-2 p-3 rounded-2xl border-2 transition-all ${currentQ.a === i ? 'border-green-500 bg-green-50' : 'border-slate-100'}`}>
                                    <input type="radio" name="ans" checked={currentQ.a === i} onChange={()=>setCurrentQ({...currentQ, a:i})} />
                ...
                <div className="space-y-4">
                    {formData.questions.map((q, i) => (
                        <div key={i} className="p-6 bg-white border rounded-[30px] shadow-sm animate-in fade-in">
                            ...
                            <div className="game-preview-options flex flex-wrap gap-2">
                                {q.options.map((opt, idx) => (
                                    <span key={idx} className={`px-4 py-2 rounded-lg text-xs font-bold ${idx === q.a ? 'bg-green-100 text-green-700 border border-green-200' : 'bg-slate-50 text-slate-400'}`}>
                                        {opt}
                                    </span>
                                ))}
                            </div>
        ...
        <div className="p-8 bg-white border-t">
            <button onClick={save} className="w-full p-8 bg-purple-600 text-white font-black text-3xl rounded-[40px] uppercase shadow-2xl hover:bg-purple-700 transition-colors">
                💾 Sauvegarder le Quiz
            </button>
        </div>
    </div>
