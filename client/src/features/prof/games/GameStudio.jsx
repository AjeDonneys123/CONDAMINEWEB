...
export default function GameStudio({ initialData, chapters, classFilter, onClose }) {
  ...
  const handleAiGen = async () => {
      ...
      try {
          ...
          console.error(e);
          alert("L'IA n'est pas accessible. Vérifiez la console serveur.");
      }
      setLoadingIA(false); 
  };

  const save = async () => {
    if (!formData.title || formData.questions.length === 0) return alert("Le quiz est vide.");
    try {
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            {currentQ.options.map((opt, i) => (
                                <div key={i} className={`flex items-center gap-2 p-3 rounded-2xl border-2 transition-all ${currentQ.a === i ? 'border-green-500 bg-green-50' : 'border-slate-100'}`}>
                                    <input type="radio" name="correct" checked={currentQ.a === i} onChange={()=>setCurrentQ({...currentQ, a:i})} />
