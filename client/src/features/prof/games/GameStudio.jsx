...
const DEFAULT_QUIZ_DATA = { title: '', chapterId: '', teacherId: null, targetClassrooms: [], assignedStudents: [], isAllClass: true, questions: [{ q: 'Nouvelle question', options: ['', '', '', ''], a: 0 }] };

export default function GameStudio({ initialData, chapters, classFilter, user, targetSection, onClose }) {
    // ✅ FIX V18:50 : Utilisation robuste de StudioUtils
    const [formData, setFormData] = useState(initialData || { ...DEFAULT_QUIZ_DATA, teacherId: user.id || user._id, targetClassrooms: classFilter ? [classFilter] : [] });
    ...
    const handleAddQuestion = () => { setFormData(p => ({ ...p, questions: [...p.questions, { q: `Nouvelle question ${p.questions.length + 1}`, options: ['', '', '', ''], a: 0 }] })); setActiveQIndex(formData.questions.length); };
    const updateQuestion = (field, value, optionIndex = null) => { setFormData(p => { const nextQuestions = [...p.questions]; const currentQ = { ...nextQuestions[activeQIndex] }; if (field === 'q' || field === 'a') { currentQ[field] = value; } else if (field === 'options' && optionIndex !== null) { const nextOptions = [...currentQ.options]; nextOptions[optionIndex] = value; currentQ.options = nextOptions; } nextQuestions[activeQIndex] = currentQ; return { ...p, questions: nextQuestions }; }); };
    const handleGenerateAI = async () => { if (!aiTopic) return alert("Entrez un sujet !"); setAiGenerating(true); try { const result = await api.post('/games/generate', { topic: aiTopic, count: 5 }); const cleaned = Array.isArray(result) ? result : []; if (cleaned.length === 0) { alert("Échec génération."); setAiGenerating(false); return; } setFormData(p => ({ ...p, questions: [...p.questions, ...cleaned.filter(q => q.q && q.options && q.options.length === 4 && q.a !== undefined)] })); setActiveQIndex(formData.questions.length); } catch(e) { alert("Erreur IA."); } setAiGenerating(false); };
    ...
    // ✅ FIX V18:50 : Utilisation de StudioUtils via l'import
    ...
    // ✅ FIX V18:50 : getChaptersForContext via StudioUtils
    ...
    return (
        <div className="v84-game-container">
            ...
            <div className="v84-game-body">
                <div className="v84-q-list-sidebar custom-scrollbar"><span className="v84-sidebar-label">Questions ({formData.questions.length})</span>{formData.questions.map((q, index) => (<div key={index} className={`v84-q-item ${activeQIndex === index ? 'active' : ''}`} onClick={() => handleSelectQuestion(index)}><div className="v84-q-preview">Q{index + 1}: {q.q.substring(0, 30)}...</div><div className="v84-q-sub">Rép: {q.options[q.a]}</div></div>))}<button className="v84-add-q-btn" onClick={handleAddQuestion} disabled={loading}>+ AJOUTER</button></div>
                <div className="v84-game-editor custom-scrollbar">
                    <div className="v84-ai-widget"><span className="text-3xl">🤖</span><input className="v84-ai-input" placeholder="Sujet..." value={aiTopic} onChange={e => setAiTopic(e.target.value)} disabled={aiGenerating || loading} /><button className="v84-ai-btn" onClick={handleGenerateAI} disabled={aiGenerating || loading}>{aiGenerating ? '...' : 'GÉNÉRER'}</button></div>
                    {currentQ && (<div className="v84-q-card"><span className="v84-field-label">Question {activeQIndex + 1}</span><textarea className="v84-q-input" value={currentQ.q} onChange={e => updateQuestion('q', e.target.value)} rows="3" disabled={loading}/><div className="v84-answers-grid">{currentQ.options.map((opt, index) => (<div key={index} className={`v84-ans-row ${currentQ.a === index ? 'correct' : ''}`}><div className="v84-correct-radio" onClick={() => updateQuestion('a', index)}>{currentQ.a === index ? '✓' : ''}</div><input className="v84-ans-input" placeholder={`Opt ${index + 1}`} value={opt} onChange={e => updateQuestion('options', e.target.value, index)} disabled={loading}/></div>))}</div></div>)}
                </div>
                <div className="v84-dist-sidebar custom-scrollbar">
                    <span className="v84-sidebar-label">Distribution</span>
                    ...
                    {viewingClass && (
                        <div className="flex-1 flex flex-col bg-slate-50 rounded-2xl overflow-hidden border border-slate-200 p-4">
                            <div className="flex justify-between items-center mb-4 cursor-pointer" onClick={toggleAllStudents}><span className="font-black text-slate-700 uppercase">{viewingClass}</span><div className={`w-5 h-5 rounded border-2 flex items-center justify-center ${isSelected ? 'bg-purple-600 border-purple-600' : 'border-slate-300'}`}>{isSelected && <span className="text-white text-xs">✓</span>}</div></div>
                            ...
                            <div className="relative mb-2">
                                ...
                                <input 
                                    className="w-full pl-8 pr-3 py-2 bg-white border border-slate-200 rounded-lg text-[10px] font-bold outline-none focus:border-purple-400" 
                    ...
                    <button className="v84-game-publish-btn" onClick={handleSave} disabled={loading || formData.questions.length === 0}>{loading ? '...' : (initialData ? 'MODIFIER' : 'PUBLIER 🚀')}</button>
                </div>
            </div>
        </div>
    );
