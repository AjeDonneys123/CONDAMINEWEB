...
/**
 * ⚙️ DASHBOARD ADMIN V111 (SUB-TABS CLASSES)
 * Ajout d'une barre de navigation secondaire dans l'onglet ÉLÈVES
 * pour filtrer par classe (1D, 2A...) au lieu d'une liste unique.
 */
export default function AdminDashboard({ user, onRefresh }) {
    ...
    // FILTRE SOUS-ONGLET (Pour les élèves)
    ...
    const loadData = async () => {
        ...
        try {
            ...
            // Tri alphabétique des classes pour les onglets
            const sortedClasses = (rC || []).sort((a,b) => a.name.localeCompare(b.name));
    ...
    useEffect(() => { 
        loadData(); 
        // Reset du filtre classe quand on change de vue principale
    ...
    const handleFileSelect = async (e) => {
        ...
        if (!file) return;
        if(!confirm(`Importer les élèves depuis "${file.name}" ?\nLe nom du fichier SERA le nom de la classe.`)) return;
        ...
        } catch (err) { alert("Erreur réseau upload"); }
        setImporting(false);
        e.target.value = null;
    };
    ...
    const openCreate = () => {
        const template = view === 'teachers' ? { firstName: '', lastName: '', password: 'A', taughtSubjects: [], assignedClasses: [] } : 
                         view === 'classes' ? { name: '', type: 'CLASS' } :
                         view === 'groups' ? { name: '', type: 'GROUP' } :
                         view === 'subjects' ? { name: '', color: '#000000' } :
                         { firstName: '', lastName: '', classId: '', assignedGroups: [] }; 
    ...
    const handleSave = async () => {
        ...
        if (res.ok) { setModalMode(null); loadData(); if(['classes','groups','teachers'].includes(view)) onRefresh(); } 
    ...
    // CALCUL DES ITEMS À AFFICHER (FILTRAGE SOUS-ONGLETS)
    const getDisplayedItems = () => {
        if (view === 'students' && activeClassTab !== 'ALL') {
            return items.filter(s => String(s.classId) === String(activeClassTab));
        }
        return items;
    };
    ...
    return (
        <div className="admin-container animate-in fade-in">
            ...
            {importing && (
                <div className="zoom-overlay" style={{zIndex: 20000}}>
                    <div className="text-white font-black text-2xl animate-pulse text-center">
                        ...
                        <div className="space-y-4 mb-6">
                            {['teachers', 'students', 'staff'].includes(view) && (
                                <div className="flex gap-4">
                                    <input className="admin-input" placeholder="Prénom" value={currentItem.firstName} onChange={e => setCurrentItem({...currentItem, firstName: e.target.value})} />
                                    <input className="admin-input" placeholder="Nom" value={currentItem.lastName} onChange={e => setCurrentItem({...currentItem, lastName: e.target.value})} />
                                </div>
                            )}
                            ...
                            {['classes', 'groups', 'subjects'].includes(view) && <input className="admin-input" placeholder="Nom" value={currentItem.name} onChange={e => setCurrentItem({...currentItem, name: e.target.value})} />}
                        </div>
                            ...
                            <div className="grid grid-cols-3 gap-4 border-t pt-6 h-[400px]">
                                <div className="flex flex-col gap-2 overflow-hidden">
                                    ...
                                    <div className="overflow-y-auto custom-scrollbar flex-1 space-y-1 bg-slate-50 p-2 rounded-xl">
                                        {allSubjects.map(s => <button key={s._id} onClick={() => toggleArrayItem('taughtSubjects', s._id)} className={`w-full text-left px-3 py-2 rounded-lg text-[10px] font-bold transition-all ${currentItem.taughtSubjects?.includes(s._id) ? 'bg-indigo-600 text-white' : 'bg-white text-slate-400'}`}>{s.name}</button>)}
                                    </div>
                                </div>
                                <div className="flex flex-col gap-2 overflow-hidden">
                                    <h4 className="text-[10px] font-black uppercase text-emerald-500">🏫 Classes</h4>
                                    <div className="overflow-y-auto custom-scrollbar flex-1 space-y-1 bg-slate-50 p-2 rounded-xl">
                                        {allClasses.filter(c => c.type === 'CLASS').map(c => <button key={c._id} onClick={() => toggleArrayItem('assignedClasses', c._id)} className={`w-full text-left px-3 py-2 rounded-lg text-[10px] font-bold transition-all ${currentItem.assignedClasses?.includes(c._id) ? 'bg-emerald-500 text-white' : 'bg-white text-slate-400'}`}>{c.name}</button>)}
                                    </div>
                                </div>
                                <div className="flex flex-col gap-2 overflow-hidden">
                                    <h4 className="text-[10px] font-black uppercase text-orange-500">👥 Groupes</h4>
                                    <div className="overflow-y-auto custom-scrollbar flex-1 space-y-1 bg-slate-50 p-2 rounded-xl">
                                        {allClasses.filter(c => c.type === 'GROUP').map(c => <button key={c._id} onClick={() => toggleArrayItem('assignedClasses', c._id)} className={`w-full text-left px-3 py-2 rounded-lg text-[10px] font-bold transition-all ${currentItem.assignedClasses?.includes(c._id) ? 'bg-orange-500 text-white' : 'bg-white text-slate-400'}`}>{c.name}</button>)}
                                    </div>
                                </div>
                            </div>
                        )}
                        ...
                        {view === 'students' && (
                            <div className="grid grid-cols-2 gap-4 border-t pt-6 h-[400px]">
                                ...
                                <div className="flex flex-col gap-2 overflow-hidden">
                                    ...
                                    <div className="overflow-y-auto custom-scrollbar flex-1 space-y-1 bg-slate-50 p-2 rounded-xl">
                                        {allClasses.filter(c => c.type === 'GROUP').map(c => <button key={c._id} onClick={() => toggleArrayItem('assignedGroups', c._id)} className={`w-full text-left px-3 py-2 rounded-lg text-[10px] font-bold transition-all ${currentItem.assignedGroups?.includes(c._id) ? 'bg-orange-500 text-white' : 'bg-white text-slate-400'}`}>{c.name}</button>)}
                                    </div>
                                </div>
                            </div>
                        )}
            ...
            <div className="flex flex-col gap-4 mb-6">
                ...
                {/* --- SOUS-MENU SPÉCIAL ÉLÈVES (CLASSE TABS) --- */}
                {view === 'students' && (
                    <div className="flex gap-2 overflow-x-auto no-scrollbar pb-2 px-2">
                        <button 
                            onClick={() => setActiveClassTab('ALL')} 
                            className={`px-4 py-2 rounded-xl text-[10px] font-black whitespace-nowrap transition-all border ${activeClassTab === 'ALL' ? 'bg-indigo-600 text-white border-indigo-600 shadow-md' : 'bg-white text-slate-400 border-slate-200'}`}
                        >
                            TOUS
                        </button>
                        {allClasses.filter(c => c.type === 'CLASS').map(c => (
                            <button 
                                key={c._id} 
                                onClick={() => setActiveClassTab(c._id)} 
                                className={`px-4 py-2 rounded-xl text-[10px] font-black whitespace-nowrap transition-all border ${activeClassTab === c._id ? 'bg-indigo-600 text-white border-indigo-600 shadow-md' : 'bg-white text-slate-400 border-slate-200'}`}
                            >
                                {c.name}
                            </button>
                        ))}
            ...
            <div className="bg-white rounded-[40px] border shadow-sm overflow-hidden min-h-[400px]">
                        ...
                        {displayedItems.map(it => {
                            ...
                            return (
                                <tr key={it._id} className="border-t hover:bg-slate-50 transition-colors">
                                    <td className="p-6">
                                        <div className="font-black text-slate-700 uppercase text-sm">{it.name || `${it.firstName} ${it.lastName}`}</div>
                                        <span className="text-[8px] font-black text-slate-300 uppercase block mb-1">
                                            {it.type || it.role || (it.classId ? allClasses.find(c=>c._id===it.classId)?.name : 'PROFIL')}
                                        </span>
                                        {isTeacher && (
                                        ...
                                        {it.assignedGroups?.length > 0 && (
                                            <div className="flex flex-wrap gap-1 mt-1">
                                                {it.assignedGroups.map(gid=>{const g=allClasses.find(c=>c._id===gid); return g ? <span key={gid} className="px-1.5 py-0.5 bg-orange-50 text-orange-600 text-[8px] rounded font-bold border border-orange-100">{g.name}</span> : null})}
                                            </div>
                                        )}
                                    </td>
                                    <td className="p-6 text-right">
                                        ...
                                        <button onClick={() => handleDelete(it._id)} className="ml-2 w-8 h-8 rounded-lg bg-red-50 text-red-500 font-bold hover:bg-red-100">✕</button>
                                    </td>
                                </tr>
                            );
                        })}
                ...
                {displayedItems.length === 0 && <div className="p-20 text-center text-slate-300 font-bold italic">Aucun élément trouvé.</div>}
