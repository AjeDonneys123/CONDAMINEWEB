...
export default function AdminDashboard({ user, onRefresh }) {
    ...
    const getActiveContext = () => {
        if (activeClassTab && activeClassTab !== 'ALL') {
            const cls = allClasses.find(c => c._id === activeClassTab);
            return cls ? { name: cls.name, level: cls.level, id: cls._id } : null;
        }
        return null;
    };
    ...
    const handleOpenCreate = () => {
        let defaults = { 
            firstName: '', lastName: '', email: '', parentEmail: '', 
            classId: '', currentLevel: '', assignedGroups: [], 
            name: '', level: '', type: 'CLASS',
            password: '', taughtSubjects: [], assignedClasses: [] 
        };
    ...
    const handleMagicImport = async () => {
        if (!magicText || magicText.length < 10) return alert("Collez les données !");
        setImporting(true);
        try {
            const res = await fetch('/api/admin/import-magic', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ rawText: magicText, defaultClass: magicClass.toUpperCase(), forceOption: magicForceOption }) });
            const data = await res.json();
            if (res.ok) { alert("✅ " + data.message); setShowMagicModal(false); loadData(); onRefresh(); } else { alert("❌ " + data.error); }
    ...
    const toggleArrayItem = (field, id) => { const list = currentItem[field] || []; setCurrentItem({ ...currentItem, [field]: list.includes(id) ? list.filter(x => x !== id) : [...list, id] }); };
    ...
    return (
        <>
                                ...
                                <div className="grid grid-cols-2 gap-4 border-t pt-4 flex-1 min-h-0">
                                    <div className="flex flex-col gap-2 h-full overflow-hidden">
                                        <div className="flex flex-col gap-1">
                                            ...
                                            <input className="w-full p-2 text-[10px] font-bold border-2 border-emerald-50 rounded-xl bg-slate-50 focus:bg-white focus:border-emerald-400 outline-none transition-colors" placeholder="🔎 Filtrer Classe..." value={filterClass} onChange={e=>setFilterClass(e.target.value)} />
                                        </div>
                                        <div className="flex-1 overflow-y-auto bg-white p-2 rounded-xl border-2 border-slate-100 custom-scrollbar flex flex-col gap-1 shadow-inner">
                                    ...
                                    <div className="flex flex-col gap-2 h-full overflow-hidden">
                                        <div className="flex flex-col gap-1">
                                            ...
                                            <input className="w-full p-2 text-[10px] font-bold border-2 border-orange-50 rounded-xl bg-slate-50 focus:bg-white focus:border-orange-400 outline-none transition-colors" placeholder="🔎 Filtrer Groupe..." value={filterGroup} onChange={e=>setFilterGroup(e.target.value)} />
                                        </div>
                                        <div className="flex-1 overflow-y-auto bg-white p-2 rounded-xl border-2 border-slate-100 custom-scrollbar flex flex-col gap-1 shadow-inner">
            ...
            {/* LE RESTE DU DASHBOARD RESTE DANS LE FLUX NORMAL */}
                ...
                <div className="flex flex-col gap-4 mb-6">
                    <div className="flex justify-between items-center bg-white p-4 rounded-[30px] shadow-sm">
                        ...
                        <div className="flex gap-2">
                            ...
                            {(view === 'classes' || view === 'groups' || view === 'students') && <button onClick={handleOpenMagic} className="bg-purple-600 text-white px-6 py-2.5 rounded-2xl font-black text-[10px] uppercase shadow-lg animate-pulse">🔮 IMPORT MAGIQUE</button>}
