    ...
    if (view === "list") return (
        <div className="p-6 space-y-6 animate-in fade-in">
            ...
            <div className="grid grid-cols-1 gap-3">
                {scans.map(s => (
                    <div key={s._id} className="bg-white p-4 rounded-2xl border shadow-sm flex justify-between">
                        <b>Note: {s.grade}</b><span>{new Date(s.createdAt).toLocaleDateString()}</span>
                    </div>
                ))}
            </div>
        </div>
    ...
    return (
        <div className="min-h-screen bg-slate-50 flex flex-col">
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-6 pb-32">
                    ...
                    <div className="bg-white p-6 rounded-[35px] shadow-lg border-2 border-indigo-100 space-y-6 animate-in zoom-in">
                        ...
                        <div className="space-y-3">
                            <div className="flex justify-between items-center px-2">
                                <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Consignes (Micro/Texte)</label>
                        ...
                        <div className="space-y-3 pt-4 border-t border-slate-100">
                            <label className="text-[10px] font-black text-blue-500 uppercase ml-2">Sujet / Questions (Images)</label>
                ...
                <div className="relative rounded-[40px] overflow-hidden bg-black aspect-[3/4] shadow-2xl border-4 border-white">
                    ...
                    <div className="absolute inset-x-0 bottom-6 flex justify-center">
                        <button onClick={takeSnap} className="w-16 h-16 bg-white rounded-full border-4 border-white/20 active:scale-90 shadow-xl transition-all"></button>
                    </div>
            ...
            <div className="p-6 bg-white border-t-2 border-slate-100 sticky bottom-0 z-[100]">
                <button onClick={startCorrection} disabled={loading || queue.length === 0} className="w-full py-6 bg-emerald-500 text-white rounded-[30px] font-black text-xl shadow-xl">
