    ...
    const toggleRecording = async () => {
        if (!isRecording) {
            try {
                ...
                setIsRecording(true);
            } catch(err) { alert("Micro bloqué ou non disponible."); }
        } else {
    ...
    if (view === "list") return (
        <div className="p-6 space-y-6 animate-in fade-in">
            ...
            <div className="space-y-3">
                {scans.map(s => (
                    <div key={s._id} className="bg-white p-4 rounded-2xl border shadow-sm flex justify-between items-center">
                        <b>Note: {s.grade}</b>
    ...
    return (
        <div className="min-h-screen bg-slate-50 flex flex-col animate-in slide-in-from-right">
            ...
            <div className="bg-white p-6 border-b-4 border-indigo-600 flex justify-between items-center sticky top-0 z-[100]">
                <button onClick={() => setView("list")} className="font-bold text-slate-400">ANNULER</button>
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-6 pb-32">
                {/* ZONE CONSIGNES (TOUJOURS VISIBLE) */}
                <div className="bg-white p-6 rounded-[35px] shadow-lg border-2 border-indigo-100 space-y-4">
                    <div className="flex justify-between items-center px-2">
                        <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Instructions (Micro / Texte)</label>
                    ...
                    <textarea 
                        ...
                        placeholder="Parle ou écris tes consignes ici..."
                    />
                    <button 
                    ...
                    >
                        {isRecording ? '🛑 ARRÊTER L\'ÉCOUTE' : '🎤 CLIQUER POUR PARLER'}
                    </button>
                </div>

                {showSettings && (
                    <div className="bg-white p-6 rounded-[35px] shadow-lg border-2 border-blue-100 space-y-4 animate-in zoom-in">
                        <label className="text-[10px] font-black text-blue-500 uppercase">Sujet / Documents (Photos)</label>
                ...
                <div className="relative rounded-[40px] overflow-hidden bg-black aspect-[3/4] shadow-2xl border-4 border-white">
                    ...
                    {!showCamera && <div onClick={startCamera} className="absolute inset-0 flex flex-col items-center justify-center bg-black/60 text-white gap-4"><b>Ouvrir Scanner</b></div>}
            ...
            <div className="p-6 bg-white border-t-2 border-slate-100 sticky bottom-0 z-[100]">
                <button onClick={startCorrection} disabled={loading || queue.length === 0} className="w-full py-6 bg-emerald-500 text-white rounded-[30px] font-black text-xl shadow-xl active:scale-95 transition-all">
