...
export default function ScansStudio({ globalClass }) {
    ...
    const deleteSession = async (id) => {
        if (!confirm("Voulez-vous vraiment supprimer cette session de scan ?")) return;
        try {
            const res = await fetch(`/api/scan-sessions/${id}`, {
                method: 'DELETE'
            });
                            ...
                            <div className="p-4 flex items-center justify-between">
                                <div className="flex items-center gap-4 cursor-pointer flex-1" onClick={() => setOpenId(isOpen ? null : s._id)}>
                                    ...
                                    <div>
                                        ...
                                        {assigned && <span className="ml-2 text-[9px] font-black text-emerald-500 uppercase px-2 py-0.5 bg-emerald-50 rounded-md">📁 {assigned.title}</span>}
                                    </div>
                                </div>
                                <div className="flex gap-2">
                                    ...
                                    <button onClick={() => deleteSession(s._id)} className="p-2 text-slate-300 hover:text-red-500 font-bold transition-colors">✕</button>
                                </div>
                            </div>

                            {showFolderPicker === s._id && (
                                <div className="p-4 bg-emerald-50 border-t flex flex-wrap gap-2 animate-in slide-in-from-top-2">
                                    <p className="w-full text-[10px] font-black text-emerald-800 uppercase px-2 mb-2">Choisir le dossier de destination :</p>
                                    {availableChapters.map(c => <button key={c._id} onClick={() => handleAssign(s._id, c._id)} className="px-3 py-2 bg-white border-2 border-emerald-200 rounded-xl text-[10px] font-bold">📁 {c.title}</button>)}
        ...
        return (
            <div className="space-y-6 animate-in zoom-in">
                <div className="p-6 bg-white rounded-3xl border-2 border-emerald-100">
                    ...
                    <textarea className="w-full h-32 p-4 bg-slate-50 rounded-2xl border-none outline-none font-medium" placeholder="Instructions..." defaultValue={session.teacherInstruction} />
                </div>
                <button className="w-full py-6 bg-emerald-600 text-white rounded-[30px] font-black text-xl shadow-xl">🚀 ANALYSER</button>
            </div>
        );
    }
