    ...
    return (
        <div className="p-2 space-y-4 max-w-5xl mx-auto sm:p-6">
            <div className="flex gap-2 overflow-x-auto pb-2 custom-scrollbar">
                {classes.map(c => (
                    <button key={c} onClick={() => setSelectedClass(c)} className={`px-6 py-3 rounded-2xl font-black text-xs transition-all flex-shrink-0 ${selectedClass === c ? 'bg-indigo-600 text-white shadow-lg' : 'bg-white text-slate-400 border'}`}>{c}</button>
                ))}
            </div>
            ...
            <div className="space-y-2">
                        ...
                        <div key={s._id} className={`bg-white rounded-[25px] border-2 transition-all ${isLocalOpen ? 'border-indigo-500 shadow-xl' : 'border-slate-50 shadow-sm'}`}>
                            <div className="p-2 flex flex-col lg:flex-row lg:items-center justify-between gap-2">
                                ...
                                <div className="flex items-center justify-around gap-1 px-2">
                                    ...
                                    <button onClick={() => { setOpenId(s._id); setActiveTab('scan'); }} className={`px-3 py-2 rounded-xl font-black text-[8px] uppercase ${isLocalOpen && activeTab==='scan' ? 'bg-indigo-600 text-white shadow-md' : 'bg-indigo-50 text-indigo-600'}`}>📄 Scan</button>
                                    <button onClick={() => { setOpenId(s._id); setActiveTab('docs'); }} className={`px-3 py-2 rounded-xl font-black text-[8px] uppercase ${isLocalOpen && activeTab==='docs' ? 'bg-indigo-600 text-white shadow-md' : 'bg-emerald-50 text-emerald-600'}`}>📂 Copies {s.copyUrls?.length || 0}</button>
                                    <button onClick={() => { setOpenId(s._id); setActiveTab('ia'); }} className={`px-3 py-2 rounded-xl font-black text-[8px] uppercase ${isLocalOpen && activeTab==='ia' ? 'bg-indigo-600 text-white shadow-md' : 'bg-slate-50 text-slate-400'}`}>🤖 IA</button>
                ...
                {filteredSessions.length === 0 && <p className="text-center py-20 text-slate-300 italic text-xs uppercase tracking-widest">Aucun scan en cours pour {selectedClass}</p>}
            </div>
        </div>
    );
}
...
function PilotArea({ currentSession, tab, onClose, onRefresh }) {
    ...
    const takePhoto = async () => {
        ...
        if (updated._id) { 
            setSession(updated); onRefresh(); 
            setTimeout(() => setLocalPreviews(p => p.filter(x => x.id !== tempId)), 1500); 
        }
    };
    ...
    const deletePhoto = async (url, type) => {
        ...
        setSession({ ...session, [field]: session[field].filter(u => u !== url) });
    ...
    return (
        <div className="space-y-4">
                ...
                <div className="space-y-4">
                    ...
                    <div className="flex gap-2 overflow-x-auto py-2">
                        {localPreviews.filter(p => p.type === (tab==='quest'?'quest':'scan')).map(p => (
                            <img key={p.id} src={p.src} className="h-24 w-16 object-cover rounded-xl border-2 border-indigo-400 animate-pulse" alt="local" />
                        ))}
                    </div>
                </div>
            )}
            ...
            {tab === 'docs' && (
                <div className="space-y-4">
                    <h4 className="font-black text-emerald-600 text-[10px] uppercase tracking-widest px-2">Dossier Drive ({session.copyUrls?.length || 0})</h4>
                    <div className="grid grid-cols-3 sm:grid-cols-4 gap-3">
                        {session.copyUrls?.map((id, i) => (
                            <div key={i} className="relative aspect-[3/4]">
                                <img src={getImgSrc(id)} className="w-full h-full object-cover rounded-2xl border-2 border-white shadow-md bg-slate-100" alt="drive" />
                                <button onClick={() => deletePhoto(id, 'copy')} className="absolute -top-2 -right-2 bg-red-500 text-white w-6 h-6 rounded-full text-xs font-bold shadow-md">✕</button>
                            </div>
                        ))}
                    </div>
                </div>
            ...
            {tab === 'ia' && (
                <textarea 
                    className="w-full p-6 h-40 bg-white rounded-[25px] border-2 border-indigo-100 outline-none font-medium shadow-inner" 
