...
export default function StudentsManager({ globalClass }) {
  ...
  return (
    <div className="manager-container">
                    ...
                    <tr key={p._id} className="border-b hover:bg-slate-50 transition-colors">
                        ...
                        <td className="p-5 text-right pr-8">
                            <button 
                                onClick={() => viewWork(p)} 
                                disabled={loading}
                                className="px-4 py-2 bg-indigo-50 text-indigo-600 rounded-xl text-[10px] font-black uppercase shadow-sm"
                            >
                                {loading && selectedStudent?.player._id === p._id ? '...' : '📂 Ouvrir'}
                            </button>
                        </td>
                    </tr>
                ))}
      ...
      {/* MODALE TRAVAUX ÉLÈVE (PORTFOLIO DRIVE) */}
      {selectedStudent && (
          <div className="fixed inset-0 z-[7000] bg-slate-900/80 backdrop-blur-sm flex items-center justify-center p-6">
              <div className="bg-white w-full max-w-2xl rounded-[40px] shadow-2xl flex flex-col max-h-[85vh] animate-in zoom-in">
                  <div className="p-8 border-b flex justify-between items-center bg-slate-50 rounded-t-[40px]">
                      <div className="flex flex-col">
                        <h2 className="text-xl font-black uppercase text-indigo-600">{selectedStudent.player.firstName} {selectedStudent.player.lastName}</h2>
                        <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Dossier Élèves / {selectedStudent.player.firstName}</span>
                      </div>
                  ...
                  <div className="flex-1 overflow-y-auto p-6 space-y-4 custom-scrollbar">
                      {selectedStudent.files.map(f => (
                          <div key={f.id} className="p-4 bg-slate-50 rounded-2xl flex items-center justify-between group hover:bg-white transition-all border border-transparent hover:border-indigo-100 shadow-sm">
                              ...
                              <a href={f.webViewLink} target="_blank" rel="noreferrer" className="bg-white px-4 py-2 rounded-xl text-indigo-500 font-black text-[10px] uppercase border shadow-sm hover:bg-indigo-600 hover:text-white transition-all">Voir Drive ➔</a>
                          </div>
                      ))}
                      {selectedStudent.files.length === 0 && (
                          <div className="text-center py-20">
                              <span className="text-4xl block mb-2">📁</span>
                              <p className="text-slate-400 italic text-sm">Le dossier de cet élève est vide pour le moment.</p>
                          </div>
                      )}
                  </div>
              </div>
