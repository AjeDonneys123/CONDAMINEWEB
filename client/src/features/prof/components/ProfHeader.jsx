...
export default function ProfHeader({ user, onLogout }) {
  ...
  return (
    <div className="p-8 pb-4 flex justify-between items-center bg-white relative border-b border-slate-50">
      <div className="flex flex-col text-left">
        ...
        <div className="flex items-center gap-2 mt-1 cursor-pointer group" onClick={checkDrive}>
            ...
            <span className={`text-[10px] font-black uppercase tracking-widest ${ (driveStatus.ok && driveStatus.isPro) ? 'text-emerald-600' : 'text-red-500'}`}>
                Drive : {driveStatus.loading ? '...' : (driveStatus.isPro ? driveStatus.email : `🚨 ERREUR`)}
            </span>
        </div>
      </div>
      <div className="flex gap-3">
          <button onClick={() => setShowDocs(true)} className="bg-indigo-50 text-indigo-600 px-4 py-2 rounded-2xl font-black text-[10px] uppercase shadow-sm border border-indigo-100 hover:bg-indigo-600 hover:text-white transition-all">📖 DOCS</button>
      ...
      {showDocs && <SystemDocs onClose={() => setShowDocs(false)} />}
    </div>
  );
}