    ...
    return (
        <div className="space-y-4">
            ...
            {filteredChapters.map(chap => {
                ...
                return (
                    <div key={chap._id} className="bg-white rounded-[35px] border-2 border-slate-50 overflow-hidden shadow-sm">
                        <button 
                            onClick={() => setOpenChaps({...openChaps, [chap._id]: !isOpen})}
                            className={`w-full p-6 flex justify-between items-center ${isOpen ? 'bg-slate-50' : ''}`}
                        >
                            <div className="flex items-center gap-4">
                                <span className="text-xl">📂</span>
                                <div className="text-left">
                                    <h3 className="font-black text-slate-700 uppercase text-sm leading-tight">{chap.title}</h3>
                                    <span className="text-[9px] font-black text-slate-400 uppercase tracking-widest">{chapItems.length} ÉLÉMENT(S)</span>
                                </div>
                            </div>
                                ...
                                {chapItems.map(it => (
                                    <div key={it._id} className="bg-white p-4 px-6 rounded-2xl flex justify-between items-center shadow-sm border border-slate-100">
                                        ...
                                        <div className="flex gap-2">
                                            ...
                                            <button 
                                                onClick={() => onDeleteItem(it._id, it.actType)}
