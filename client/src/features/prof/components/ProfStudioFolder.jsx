...
/**
 * 📁 PROF STUDIO FOLDER - RESTAURATION V465
 * Réparation de la création de section avec choix de portée (Global/Niveau/Classe).
 */
export default function ProfStudioFolder({ items, chapters, studentsRef, classFilter, levelFilter, user, onEditItem, onCreateActivity, onRefresh }) {
    const [customSections, setCustomSections] = useState([]);
    const [activeSection, setActiveSection] = useState("GÉNÉRAL"); 
    const [openChaps, setOpenChaps] = useState({}); 
    const [showArchived, setShowArchived] = useState(false); 
    
    // --- ÉTATS MODALE CRÉATION SECTION ---
    const [showSectionModal, setShowSectionModal] = useState(false);
    const [newSectionName, setNewSectionName] = useState("");
    const [newSectionScope, setNewSectionScope] = useState("GLOBAL"); 
    
    // --- ÉTATS MODALE CRÉATION DOSSIER ---
    ...
    // --- LOGIQUE SECTIONS (CRÉATION AVEC PORTÉE) ---

    async function handleCreateSection() {
        if (!newSectionName) return;
        
        // Calcul de la cible en fonction du scope choisi
    ...
    function handleRenameChapter(e, chapId, oldTitle) {
        ...
        // Déduction du scope actuel pour pré-remplir
    ...
    function prepareDelete(e, item, type) {
        ...
        if (type === 'chapter' && activeSection.toUpperCase() === "GÉNÉRAL" && name.trim().toUpperCase() === "GÉNÉRAL") {
            const sectionChapters = safeChapters.filter(c => c.section.toUpperCase() === "GÉNÉRAL" && !c.isArchived);
            if (sectionChapters.length <= 1) return alert("🔒 PROTECTION : Racine indestructible.");
        }
        ...
        else isShared = (item.targetClassrooms && item.targetClassrooms.length > 1);
    ...
    async function executeDelete(id, type, permanent) {
        const uid = getUserId();
        const url = (type === 'section') ? '/api/structure/sections/delete-request' : (type === 'chapter') ? '/api/structure/chapters/delete-request' : '/api/structure/activity/delete-request';
    ...
    // FILTRAGE DES DOSSIERS À AFFICHER
    const filteredChapters = safeChapters.filter(c => {
        // 1. Filtrer par section et archive
        if (c.section.toUpperCase() !== activeSection.toUpperCase() || c.isArchived !== showArchived) return false;
        
        // 2. Filtrer les masqués
        if (c.hiddenIn && c.hiddenIn.includes(classFilter)) return false;
        
        // 3. LOGIQUE DE VISIBILITÉ (Le coeur du problème)
        const isRoot = activeSection.toUpperCase() === "GÉNÉRAL" && c.title.toUpperCase() === "GÉNÉRAL";
        if (isRoot) return true; // La racine est toujours visible
        ...
        // Visible si: Pas de restriction (vieux dossiers) OU pour ma classe OU pour mon niveau
        return (!c.classroom && !c.sharedLevel) || isForMyClass || isForMyLevel;
    });

    const currentClassSize = classFilter && studentsRef ? studentsRef.filter(s => (s.currentClass || "").trim() === classFilter).length : 0;
        ...
        <div className="animate-in fade-in">
            ...
            <div className="p-6 rounded-b-[40px] bg-slate-900 shadow-xl overflow-visible">
                <div className="flex justify-between items-center mb-2 px-4">
                    ...
                    <div className="flex gap-2">
                        ...
                        <button onClick={() => { setNewSectionScope("GLOBAL"); setShowSectionModal(true); }} className="bg-indigo-600 text-white px-4 py-2 rounded-xl font-black text-[10px] shadow-lg">+ Section</button>
                    </div>
                </div>
                        ...
                        return (
                            <div key={idx} className="relative group shrink-0">
                                ...
                                >
                                    ...
                                    <div className="text-[7px] font-black text-white/30 mt-1 uppercase tracking-widest">{s.scope === 'GLOBAL' ? 'GLOBAL' : (s.scope === 'LEVEL' ? `NIV ${s.target}` : `CLASSE ${s.target}`)}</div>
                                </button>
                                ...
                                {!isGeneral && (
                                    <div className="absolute -top-3 -right-2 flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity z-20">
                                        <button 
                                            ...
                                            className="w-7 h-7 bg-white shadow-2xl rounded-full flex items-center justify-center text-[11px] border border-slate-200 hover:scale-110 transition-transform cursor-pointer"
                                            title="Modifier"
                                        >
                                            ✏️
                                        </button>
                                        <button 
                                            ...
                                            className="w-7 h-7 bg-red-500 text-white shadow-2xl rounded-full flex items-center justify-center text-[10px] font-black hover:scale-110 transition-transform cursor-pointer"
                                            title="Supprimer"
                                        >
                                            ✕
                                        </button>
            ...
            <div className="px-6 mt-10">
                <div className="flex justify-between items-end mb-8">
                    <h2 className="text-5xl font-black uppercase tracking-tighter" style={{ color: (safeSections.find(s=>s.name.toUpperCase()===activeSection.toUpperCase())?.color || '#64748b') }}>{activeSection}</h2>
                    {!showArchived && (
                        <div className="flex gap-2">
                ...
                <div className="grid grid-cols-1 gap-4 pb-20">
                    {filteredChapters.sort((a,b) => a.title.localeCompare(b.title)).map(chap => {
                        const isRoot = activeSection.toUpperCase() === "GÉNÉRAL" && chap.title.trim().toUpperCase() === "GÉNÉRAL";
                        const isLastSurvivor = activeSection.toUpperCase() === "GÉNÉRAL" && filteredChapters.length <= 1;

                        const relevantItems = uniqueItems.filter(it => {
                            if (String(it.chapterId) !== String(chap._id)) return false;
                            if (classFilter && it.targetClassrooms && !it.targetClassrooms.includes(classFilter)) return false;
                            return true;
                        });
                            ...
                            <div key={chap._id} className="bg-white border-2 rounded-[30px] overflow-hidden shadow-sm border-slate-100">
                                <div className="p-5 flex justify-between items-center cursor-pointer hover:bg-slate-50" onClick={() => setOpenChaps({...openChaps, [chap._id]: !openChaps[chap._id]})}>
                                    <div className="flex items-center gap-4">
                                        <div className="w-10 h-10 rounded-xl flex items-center justify-center text-white text-xl shadow-inner" style={{ backgroundColor: (safeSections.find(s=>s.name.toUpperCase()===activeSection.toUpperCase())?.color || '#64748b') }}>{openChaps[chap._id] ? '📂' : '📁'}</div>
                                        <div>
                                ...
                                {openChaps[chap._id] && (
                                    <div className="bg-slate-50/50 border-t p-4 space-y-2">
                                        {relevantItems.map(it => {
                                            const targetClassName = (it.targetClassrooms && it.targetClassrooms.length > 0) ? it.targetClassrooms[0] : "";
                                            const assignedCount = it.assignedStudents?.length || 0;
                                            const showList = !it.isAllClass && assignedCount > 0;
                                            ...
                                            return (
                                                <div key={it._id} className="bg-white p-3 rounded-2xl flex justify-between items-center shadow-sm border border-slate-100">
                                                    <div className="flex flex-col">
                                                        ...
                                                        {showList ? (
                                                            <div className="text-[8px] text-orange-500 font-bold mt-1 uppercase">
                                                                👤 {targetClassName} : {it.assignedStudents.map(id => {
                                                                    const s = studentsRef.find(st => String(st._id) === String(id));
                                                                    return s ? `${s.firstName} ${s.lastName.charAt(0)}.` : '...';
                                                                }).join(', ')}
                                                            </div>
                                                        ) : (
                                                            <div className="text-[8px] text-emerald-500 font-bold mt-1 uppercase">
                                                                🏫 Toute la {targetClassName}
                                                            </div>
                                                        )}
                                                    </div>
                                                    <div className="flex gap-2">
            ...
            {/* MODALE CRÉATION SECTION (RÉPARÉE) */}
            {showSectionModal && (
                <div className="fixed inset-0 z-[40000] flex items-center justify-center bg-slate-900/90 backdrop-blur-sm">
                    <div className="bg-white p-10 rounded-[50px] w-full max-w-lg shadow-2xl animate-in zoom-in">
                        ...
                        <div className="mb-6">
                            <label className="text-[10px] font-black text-slate-400 uppercase ml-2 block mb-1">Nom de la matière</label>
                            <input className="w-full p-5 rounded-2xl bg-slate-100 border-none font-bold outline-none focus:ring-4 ring-indigo-500/20" placeholder="ex: MATHS, HISTOIRE..." value={newSectionName} onChange={e => setNewSectionName(e.target.value)} />
                        </div>

                        <div className="mb-8">
                            <label className="text-[10px] font-black text-slate-400 uppercase ml-2 block mb-3">Visibilité (Portée)</label>
                            <div className="grid grid-cols-3 gap-3">
                                <button onClick={() => setNewSectionScope("GLOBAL")} className={`p-4 rounded-2xl font-black text-[9px] border-2 transition-all ${newSectionScope === "GLOBAL" ? 'bg-indigo-600 border-indigo-600 text-white shadow-lg' : 'bg-slate-50 text-slate-400'}`}>
                                    🌍 PARTOUT
                                </button>
                                <button onClick={() => setNewSectionScope("LEVEL")} className={`p-4 rounded-2xl font-black text-[9px] border-2 transition-all ${newSectionScope === "LEVEL" ? 'bg-indigo-600 border-indigo-600 text-white shadow-lg' : 'bg-slate-50 text-slate-400'}`}>
                                    🎓 NIVEAU {levelFilter}
                                </button>
                                <button onClick={() => setNewSectionScope("CLASS")} className={`p-4 rounded-2xl font-black text-[9px] border-2 transition-all ${newSectionScope === "CLASS" ? 'bg-indigo-600 border-indigo-600 text-white shadow-lg' : 'bg-slate-50 text-slate-400'}`}>
                                    🏫 CLASSE {classFilter}
                                </button>
                            </div>
                        </div>
                        ...
                        <div className="flex gap-4">
                            <button onClick={() => setShowSectionModal(false)} className="flex-1 p-5 rounded-2xl font-black text-xs bg-slate-100 text-slate-400 uppercase">Annuler</button>
                            <button onClick={handleCreateSection} className="flex-1 p-5 rounded-2xl font-black text-xs bg-indigo-600 text-white uppercase shadow-xl">CRÉER ✨</button>
                        </div>
                    </div>
                </div>
            )}
            ...
            {showChapterModal && (
                <div className="fixed inset-0 z-[35000] flex items-center justify-center bg-slate-900/90 backdrop-blur-sm">
            ...
            {showEditChapterModal && editingChapter && (
                <div className="fixed inset-0 z-[35000] flex items-center justify-center bg-slate-900/90 backdrop-blur-sm">
                    <div className="bg-white p-10 rounded-[50px] w-full max-w-md shadow-2xl animate-in zoom-in">
                        <h3 className="text-2xl font-black mb-2 uppercase text-slate-800">Modifier Dossier</h3>
                        ...
                        <div className="flex gap-4">
                            ...
                            <button onClick={handleUpdateChapterComplete} className="flex-1 p-5 rounded-2xl font-black text-xs bg-slate-900 text-white uppercase shadow-xl">Enregistrer</button>
                        </div>
                    </div>
                </div>
            )}
