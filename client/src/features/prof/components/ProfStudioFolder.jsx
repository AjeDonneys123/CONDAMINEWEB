...
/**
 * 📁 COMPOSANT FOLDER V460 - CRASH FIX & SAFE RENDER
 * RÔLE : Gestion des dossiers et sections.
 * FIX CRITIQUE : Vérification stricte 'Array.isArray()' avant tout .map() pour éviter l'écran blanc au démarrage.
 */
export default function ProfStudioFolder({ items, chapters, studentsRef, classFilter, levelFilter, user, onEditItem, onCreateActivity, onRefresh }) {
    ...
    // DÉDUPLICATION SÉCURISÉE (On garde les objets uniques par ID)
            ...
            <div className="px-6 mt-10">
                ...
                <div className="grid grid-cols-1 gap-4 pb-20">
                    {filteredChapters.sort((a,b) => a.title.localeCompare(b.title)).map(chap => {
                        ...
                        // 🔍 FILTRE INTELLIGENT DES ITEMS
                        const relevantItems = uniqueItems.filter(it => {
                            if (String(it.chapterId) !== String(chap._id)) return false;
                            // Si on filtre par classe, on ne compte pas les items des autres classes
                                ...
                                {openChaps[chap._id] && (
                                    <div className="bg-slate-50/50 border-t p-4 space-y-2">
                                        {relevantItems.map(it => {
                                            ...
                                            const isManuallyFullClass = classFilter && currentClassSize > 0 && assignedCount >= currentClassSize;
                                            const showList = !it.isAllClass && !isManuallyFullClass && assignedCount > 0;
                                            
                                            // 🔥 RECUPERATION DU NOM DE LA CLASSE CIBLE POUR LE BADGE
