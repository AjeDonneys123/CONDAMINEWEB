        ...
        <div className="space-y-4">
            {filteredChapters.length === 0 && (
                <div className="text-center py-20 bg-white rounded-[40px] border-2 border-dashed border-slate-100">
                    <p className="text-slate-300 font-black uppercase text-xs">Aucun dossier trouvé</p>
                </div>
            )}
                    ...
                    <div key={chap._id} className="bg-white rounded-[35px] border-2 border-slate-50 overflow-hidden shadow-sm transition-all">
                        ...
                        >
                            <div className="flex items-center gap-4 text-left">
                                ...
                                <div>
                                    ...
                                    <span className="text-[9px] font-black text-slate-400 uppercase">{chapItems.length} ÉLÉMENTS</span>
                                </div>
                            </div>
                        ...
                        {isOpen && (
                            <div className="p-4 bg-slate-50/30 border-t border-slate-100 space-y-3">
                                {chapItems.map(it => (
                                    <div key={it._id} className="bg-white p-4 px-6 rounded-2xl flex justify-between items-center shadow-sm border border-slate-100 animate-in">
                                        ...
                                        <div className="flex gap-2">
                                            <button onClick={() => onEditItem(it)} className="w-10 h-10 flex items-center justify-center bg-indigo-50 text-indigo-600 rounded-xl hover:bg-indigo-600 hover:text-white transition-all shadow-sm"><span className="font-black">✎</span></button>
                                            <button onClick={() => onDeleteItem(it._id, it.actType)} className="w-10 h-10 flex items-center justify-center bg-red-50 text-red-500 rounded-xl hover:bg-red-500 hover:text-white transition-all shadow-sm"><span className="font-black">🗑️</span></button>
                                        </div>
                                    </div>
                                ))}
                            </div>
