...
const COLOR_PALETTE = [
    '#ef4444', '#3b82f6', '#22c55e', '#f59e0b', '#8b5cf6', '#ec4899', '#06b6d4', '#f97316', '#facc15'
];
    ...
    const saveSections = async (newSections) => {
        ...
        try {
            ...
            if (res.ok && data.user) {
                ...
                if (onNotify) onNotify({ message: "Matières enregistrées" });
            }
        } catch(e) { console.error(e); }
    };
            ...
            <div key={chap._id} className="bg-white rounded-[35px] border-2 shadow-sm mb-3 relative overflow-visible transition-all" style={{ borderColor: isOpen ? color : '#f1f5f9', zIndex: movingId === chap._id ? 100 : 1 }}>
                <div className="p-4 flex items-center justify-between">
                    <div className="flex items-center gap-4 flex-1 cursor-pointer" onClick={() => setOpenChaps({...openChaps, [chap._id]: !isOpen})}>
                        <div className={`w-12 h-12 rounded-2xl flex items-center justify-center font-black text-sm ${color==='#facc15'?'text-slate-900':'text-white'}`} style={{ backgroundColor: color }}>{letter}</div>
                        <div className="flex flex-col text-left">
                            {isEditing ? (
                                <input autoFocus className="text-sm font-black border-b-2 outline-none" value={tempTitle} onChange={e => setTempTitle(e.target.value)} onBlur={() => { onRename(chap._id, tempTitle, chap.subject); setEditingId(null); }} />
                            ) : (
                    ...
                    <div className="flex gap-1 items-center">
                        <div className="relative">
                            ...
                            {movingId === chap._id && (
                                <div className="absolute right-0 top-10 z-[200] bg-white border-2 shadow-2xl rounded-2xl p-2 min-w-[150px]">
                                    {sections.map(s => (
                                        <button key={s.name} onClick={async () => { 
                                            await fetch('/api/chapters', { method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({_id:chap._id, subject:s.name, classroom:classFilter}) });
                        ...
                        <button onClick={(e) => { e.stopPropagation(); setEditingId(chap._id); setTempTitle(chap.title); }} className="w-8 h-8 flex items-center justify-center bg-slate-50 text-slate-300 rounded-full">✏️</button>
            ...
            <div className="p-8 bg-slate-900 rounded-[50px] border-4 border-slate-800 shadow-2xl">
                <div className="flex justify-between items-center mb-8 px-2">
                    ...
                    <div className="flex gap-2">
                        <button onClick={handleForceSync} disabled={isSyncing} className="bg-white text-indigo-600 px-5 py-2 rounded-2xl font-black text-[9px] uppercase shadow-lg">
                            {isSyncing ? 'Synchronisation...' : '🔄 Synchro Drive'}
                        </button>
                        <button onClick={() => { const n = prompt("Nom ?"); if(n) saveSections([...sections, {name:n, color:COLOR_PALETTE[sections.length%COLOR_PALETTE.length]}]); }} className="bg-indigo-600 text-white px-5 py-2 rounded-2xl font-black text-[9px] uppercase shadow-lg">+ Nouvelle Matière</button>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                    {sections.map((s, idx) => {
                        ...
                        const isPicking = colorPickerIdx === idx;
                                ...
                                <div className="flex justify-between items-center mb-4">
                                    ...
                                    {isDeleteMode && (
                                        <div className="flex gap-1 relative">
                                            <button onClick={() => setColorPickerIdx(isPicking?null:idx)} className="text-white text-[10px]">🎨</button>
                                            <button onClick={() => saveSections(sections.filter(x => x.name !== s.name))} className="text-red-500 text-[10px]">✕</button>
                                            {isPicking && (
                                                <div className="absolute top-6 right-0 z-[300] bg-slate-800 border p-2 rounded-xl grid grid-cols-3 gap-1 w-[120px]">
                                                    {COLOR_PALETTE.map(c => <button key={c} onClick={() => { const ns = [...sections]; ns[idx].color=c; saveSections(ns); setColorPickerIdx(null); }} className="w-6 h-6 rounded-full" style={{ background: c }} />)}
                                                </div>
                                            )}
                                        </div>
                                    )}
            ...
            <div className="space-y-16">
                    ...
                    return (
                        <div key={'active-' + s.name} className="animate-in fade-in">
                            <div className="flex items-center justify-between mb-4 px-6 border-b border-slate-100 pb-4">
                                ...
                                <button onClick={() => { const n = prompt(`Dossier dans ${s.name} ?`); if(n) onCreateChapter(s.name, n); }} className="text-[9px] font-black px-6 py-2 rounded-full border-2 border-dashed" style={{ color: s.color, borderColor: s.color }}>+ CRÉER DOSSIER</button>
                            </div>
                ...
                {orphanActive.length > 0 && (
                    <div className="animate-in fade-in pt-10">
                        <h3 className="font-black text-base uppercase text-red-500 text-left px-6 border-b border-red-100 pb-4 mb-4">⚠️ DOSSIERS À RECLASSER</h3>
