...
/**
 * 📂 PROF STUDIO FOLDER - VERSION 161 (ARCHIVE VISION)
 * Fix : Les compteurs des sections affichent désormais correctement le nombre de dossiers
 * (Actifs ou Archivés selon le mode), même pour les sections inactives.
 * UI : Design distinct pour le mode Archive.
 */
export default function ProfStudioFolder({ items, chapters, classFilter, levelFilter, user, onEditItem, onDeleteItem, onRefresh }) {
    ...
    // --- FILTRAGE NIVEAU 1 : LE CONTEXTE (Classe/Niveau/Prof) ---
    // Ce filtre prépare tous les chapitres disponibles pour cet écran, SANS filtrer par section ni par archive
    ...
    // --- FILTRAGE NIVEAU 2 : LA VUE (Section + Archive) ---
    // C'est ce qu'on affiche dans la grille
    ...
    const confirmCreateSection = async (scope) => {
        ...
        try {
            ...
            if (res.ok) { const newList = await res.json(); setCustomSections(newList); setActiveSection(newSectionName.toUpperCase().trim()); setShowSectionModal(false); setNewSectionName(""); if(onRefresh) onRefresh(); }
        } catch (e) { alert("Erreur réseau."); }
    };
    const handleDeleteSection = async (sectionName) => {
        if(!confirm(`Supprimer la section "${sectionName}" ?`)) return;
        try { const res = await fetch('/api/structure/sections', { method: 'DELETE', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ teacherId: getUserId(), sectionName }) }); if (res.ok) { const newList = await res.json(); setCustomSections(newList); if(activeSection === sectionName) setActiveSection('GÉNÉRAL'); if(onRefresh) onRefresh(); } } catch(e) { alert("Erreur réseau."); }
    };
    const handleReset = async () => {
        if(!confirm("⚠️ R.A.Z : Tout effacer et remettre 'GÉNÉRAL' ?")) return;
        try { const res = await fetch('/api/structure/sections/reset', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ teacherId: getUserId() }) }); if(res.ok) { const newList = await res.json(); setCustomSections(newList); setActiveSection('GÉNÉRAL'); if(onRefresh) onRefresh(); } } catch(e) { alert("Erreur Reset."); }
    };
    const handleCreateChapter = async () => {
        ...
        if (!title) return;
        let isShared = false;
        if (currentLevel) isShared = confirm(`Partager ce dossier avec tout le niveau ${currentLevel} ?`);
        await fetch('/api/structure/chapters', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ title: title.toUpperCase(), section: activeSection, classroom: classFilter.toUpperCase(), teacherId: getUserId(), sharedLevel: isShared ? currentLevel : null }) }); if(onRefresh) onRefresh();
    };

    // --- FONCTION DE FILTRAGE ITEM ---
    ...
    return (
        <div className="space-y-8 animate-in fade-in relative">
            {/* MODALES (Code inchangé pour économiser l'espace visuel, reprendre V152 si besoin) */}
            {showSectionModal && (
            ...
            <div className={`p-8 rounded-[45px] border-4 shadow-2xl relative transition-colors duration-500 ${showArchived ? 'bg-amber-950 border-amber-900' : 'bg-slate-900 border-slate-800'}`}>
                <div className="flex justify-between items-center mb-6 relative z-10">
                    <div className="flex items-center gap-6">
                        <h3 className="text-white font-black text-[11px] uppercase tracking-[0.3em]">
                            {showArchived ? 'ARCHIVES SECRÈTES' : 'CLOUD CONDAMINE'}
                        </h3>
                        <button onClick={() => setShowArchived(!showArchived)} 
                            className={`px-5 py-2.5 rounded-2xl text-[10px] font-black border-2 transition-all ${showArchived ? 'bg-amber-500 border-amber-400 text-white shadow-[0_0_20px_rgba(245,158,11,0.5)]' : 'bg-slate-800 border-slate-700 text-slate-400'}`}
                        >
                            {showArchived ? '📂 RETOUR ACTIFS' : `📦 VOIR ARCHIVES`}
                        </button>
                    </div>
                    <div className="flex items-center gap-2">
                        <div className="bg-indigo-600 text-white px-4 py-2 rounded-xl font-black text-[10px]">STUDIO V161</div>
                        {!showArchived && (
                            <>
                                ...
                                <button onClick={() => setShowSectionModal(true)} className="bg-white/10 text-white px-5 py-2.5 rounded-xl font-black text-[10px] uppercase border border-white/10 hover:bg-white/20 transition-all">+ Section</button>
                            </>
                        )}
                    </div>
                </div>
                ...
                <div className="flex gap-4 overflow-x-auto no-scrollbar pt-6 pb-4 relative z-10">
                    {loading ? <span className="text-white font-black animate-pulse text-xs">CHARGEMENT...</span> : displaySections.map((s, idx) => {
                        // V161 : CALCUL DU COMPTEUR BASÉ SUR LE CONTEXTE GLOBAL ET LE MODE ARCHIVE
                        const count = contextChapters.filter(c => {
                            const matchSection = (c.section || "GÉNÉRAL").toUpperCase() === s.name;
                            const matchStatus = !!c.isArchived === showArchived;
                            return matchSection && matchStatus;
                        }).length;

                        return (
                            <div key={idx} className="relative shrink-0">
                                <button onClick={() => setActiveSection(s.name)}
                                ...
                                >
                                    ...
                                    <span className={`text-[9px] font-bold uppercase ${count > 0 ? 'text-white' : 'text-slate-600'}`}>
                                        {count} Dossiers
                                    </span>
                                </button>
                                {displaySections.length >= 2 && !s.isVirtual && (
                                    <div onClick={(e) => { e.stopPropagation(); handleDeleteSection(s.name); }}
                                        className="absolute -top-3 -right-2 w-8 h-8 bg-red-500 text-white rounded-full font-black text-xs flex items-center justify-center shadow-lg cursor-pointer border-2 border-slate-900 hover:bg-red-600 hover:scale-110 transition-transform" style={{ zIndex: 9999 }}>✕</div>
                                )}
                            </div>
                        );
                    })}
                ...
                <div className="flex justify-between items-end mb-10 px-6">
                    <div>
                        ...
                        <p className="text-[11px] font-black text-slate-400 tracking-[0.2em] uppercase mt-2">
                            {displayedChapters.length} Dossiers {showArchived ? 'ARCHIVÉS' : 'ACTIFS'}
                        </p>
                    </div>
                ...
                <div className="grid grid-cols-1 gap-6">
                    ...
                    {displayedChapters.length === 0 && (
                        <div className="p-20 text-center border-4 border-dashed border-slate-100 rounded-[40px]">
                            <p className="font-black text-slate-300 text-xl uppercase">Aucun dossier {showArchived ? 'archivé' : 'actif'}</p>
                            <p className="text-xs font-bold text-slate-400 mt-2">dans la section {activeSection}</p>
                        </div>
                    )}
                </div>
            </div>
