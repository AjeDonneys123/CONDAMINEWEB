...
export default function ProfStudioFolder({ items, chapters, studentsRef, allClasses, classFilter, levelFilter, user, onEditItem, onCreateActivity, onRefresh }) {
    ...
    const handleCreateSection = async () => {
        ...
        const nameUpper = newSectionName.toUpperCase();
    ...
    const handleCreateChapter = async () => {
        ...
        setNewChapterTitle(""); setShowChapterModal(false); 
    ...
    const handleSaveChapterEdit = async () => {
        ...
        if (res.ok) { setShowEditChapterModal(false); setEditingChapter(null); setTimeout(() => { if (onRefresh) onRefresh(); }, 100); }
    };
    ...
    const prepareDelete = (e, item, type) => {
        ...
        if (!isShared) {
            if (confirm(`Supprimer ${name} ?`)) executeDelete(id, type, true, name);
        } else {
            setDeleteTarget({ id, type, name });
        }
    };

    const executeDelete = async (id, type, permanent, nameOverride = null) => {
        const uid = getUserId();
        ...
        if (type === 'section') body.sectionName = nameOverride || deleteTarget?.name || id;
    ...
    return (
        <div className="animate-in fade-in">
                                    ...
                                    <div className="flex items-center gap-4">
                                        ...
                                        <div>
                                            <div className="flex items-center gap-2">
                                                ...
                                                {chap.classroom ? <span className="bg-indigo-50 text-indigo-400 text-[7px] font-black px-2 py-0.5 rounded uppercase tracking-widest border border-indigo-100">🏫 CLASSE</span> : (!isRoot ? <span className="bg-purple-100 text-purple-600 text-[7px] font-black px-2 py-0.5 rounded uppercase tracking-widest border border-purple-200">🎓 NIVEAU</span> : null)}
                                            </div>
            ...
            {/* MODALES RESTE DU CODE (SANS CHANGEMENT) ... */}
