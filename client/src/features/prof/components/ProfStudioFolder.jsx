// @signatures: ProfStudioFolder, fetchSections, handleCreateSection, handleCreateChapter, handleRenameChapter, handleUpdateChapterComplete, handleArchiveChapter, prepareDelete, executeDelete, handleOpenEditSection, handleSaveSectionEdit
import React, { useState, useEffect } from 'react';

/**
 * 📁 COMPOSANT FOLDER V440 - UNIFICATION SUPPRESSION
 * RÔLE : Gestion des dossiers et sections avec modale de suppression unique et stylisée.
 */
export default function ProfStudioFolder({ items, chapters, studentsRef, classFilter, levelFilter, user, onEditItem, onCreateActivity, onRefresh }) {
    ...
    async function fetchSections() {
        ...
        try {
            ...
            if (res.ok) {
                ...
                let list = (data || []).filter(s => s.name.toUpperCase() !== "GÉNÉRAL");
    ...
    function handleRenameChapter(e, chapId, oldTitle) {
        e.stopPropagation();
        const chap = (chapters || []).find(c => c._id === chapId);
    ...
    function prepareDelete(e, item, type) {
        ...
        if (type === 'chapter' && activeSection.toUpperCase() === "GÉNÉRAL" && name.trim().toUpperCase() === "GÉNÉRAL") {
            const sectionChapters = chapters.filter(c => c.section.toUpperCase() === "GÉNÉRAL" && !c.isArchived);
            if (sectionChapters.length <= 1) return alert("🔒 PROTECTION : Racine indestructible.");
        }
        const id = item._id || item.id;
        
        // 🚀 UNIFICATION : On détermine si l'objet est partagé ou privé
        let isShared = false;
        ...
        // On ouvre SYSTEMATIQUEMENT la modale custom
    ...
    return (
        <div className="animate-in fade-in">
            ...
            <div className="px-6 mt-10">
                ...
                <div className="grid grid-cols-1 gap-4 pb-20">
                    {filteredChapters.sort((a,b) => a.title.localeCompare(b.title)).map(chap => {
                        ...
                        const chapItemsCount = (items || []).filter(it=>String(it.chapterId)===String(chap._id)).length;
                                ...
                                <div className="p-5 flex justify-between items-center cursor-pointer hover:bg-slate-50" onClick={() => setOpenChaps({...openChaps, [chap._id]: !openChaps[chap._id]})}>
                                    <div className="flex items-center gap-4">
                                        ...
                                        <div>
                                            ...
                                            <span className="text-[8px] font-black text-slate-400 uppercase">{chapItemsCount} ÉLÉMENTS</span>
                                        </div>
                                    </div>
                                        ...
                                        {(items || []).filter(it => String(it.chapterId) === String(chap._id)).map(it => (
                                            <div key={it._id} className="bg-white p-3 rounded-2xl flex justify-between items-center shadow-sm border border-slate-100">
                                                <div className="flex flex-col">
                                                    ...
                                                    {it.actType === 'homework' && it.isAllClass === false && it.assignedStudents?.length > 0 && (
                                                        <div className="text-[8px] text-orange-500 font-bold mt-1 uppercase">
                                                            Assigné à : {it.assignedStudents.map(id => {
                ...
                <div className="fixed inset-0 z-[40000] flex items-center justify-center bg-slate-900/90 backdrop-blur-sm">
                    <div className="bg-white p-10 rounded-[50px] w-full max-w-lg shadow-2xl animate-in zoom-in">
                        ...
                        <div className="mb-6">
                            ...
                            <input 
                                className="w-full p-5 rounded-2xl bg-slate-100 border-none font-bold outline-none focus:ring-4 ring-indigo-500/20" 
                            ...
                            <div className="grid grid-cols-6 gap-2 px-2">
                                {PRESET_COLORS.map(c => (
                                    <button 
                                        ...
                                        className={`w-10 h-10 rounded-xl border-4 transition-all ${editingSection.color === c ? 'border-slate-900 scale-110 shadow-lg' : 'border-transparent opacity-60 hover:opacity-100'}`}
                        ...
                        <div className="mb-8">
                            ...
                            <div className="grid grid-cols-3 gap-3">
                                {['GLOBAL', 'LEVEL', 'CLASS'].map(s => (
                                    <button 
                                        key={s} 
                                        onClick={() => setEditingSection({...editingSection, scope: s})} 
                                        className={`p-4 rounded-2xl font-black text-[9px] border-2 transition-all ${editingSection.scope === s ? 'bg-indigo-600 border-indigo-600 text-white shadow-lg' : 'bg-slate-50 text-slate-400'}`}
                                    >
            ...
            {/* 🚀 MODALE SUPPRESSION UNIFIÉE V440 */}
            {deleteTarget && (
            ...
            {showEditChapterModal && editingChapter && (
                <div className="fixed inset-0 z-[35000] flex items-center justify-center bg-slate-900/90 backdrop-blur-sm">
                    <div className="bg-white p-10 rounded-[50px] w-full max-w-md shadow-2xl animate-in zoom-in">
                        ...
                        <input className="w-full p-5 rounded-2xl bg-slate-100 border-none font-bold mb-6 outline-none focus:ring-4 ring-indigo-500/20" value={editingChapter.title} onChange={e => setEditingChapter({...editingChapter, title: e.target.value})} autoFocus />
                        <div className="grid grid-cols-3 gap-3 mb-8">
                            {['GLOBAL', 'LEVEL', 'CLASS'].map(s => (
                                <button key={s} onClick={() => setEditingChapter({...editingChapter, scope: s})} className={`p-4 rounded-2xl font-black text-[9px] border-2 ${editingChapter.scope === s ? 'bg-indigo-600 border-indigo-600 text-white shadow-lg' : 'bg-slate-50 text-slate-400'}`}>
                                    {s === 'GLOBAL' ? '🌍 GLOBAL' : s === 'LEVEL' ? '🎓 NIV' : '🏫 CLASSE'}
                                </button>
                            ))}
                        </div>
                        <div className="flex gap-4">
                            <button onClick={() => { setShowEditChapterModal(false); setEditingChapter(null); }} className="flex-1 p-5 rounded-2xl font-black text-xs bg-slate-100 text-slate-400 uppercase">Annuler</button>
                            <button onClick={handleUpdateChapterComplete} className="flex-1 p-5 rounded-2xl font-black text-xs bg-indigo-600 text-white uppercase shadow-xl">Enregistrer ✨</button>
                        </div>
                    </div>
                </div>
            )}
