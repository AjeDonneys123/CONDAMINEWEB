        ...
        <div className="space-y-4">
            {filteredChapters.length === 0 && (
                <div className="text-center py-20 bg-white rounded-[40px] border-2 border-dashed border-slate-100">
                    <p className="text-slate-300 font-black uppercase text-xs">Aucun dossier pour cette classe</p>
                </div>
            )}
                    ...
                    <div key={chap._id} className="bg-white rounded-[35px] border-2 border-slate-50 overflow-hidden shadow-sm transition-all">
                        ...
                        {isOpen && (
                            <div className="p-4 bg-slate-50/30 border-t border-slate-100 space-y-3">
                                ...
                                {chapItems.length === 0 && (
                                    <p className="text-center py-4 text-[9px] font-black text-slate-300 uppercase italic">Dossier vide</p>
                                )}
                            </div>
                        )}
                    </div>
