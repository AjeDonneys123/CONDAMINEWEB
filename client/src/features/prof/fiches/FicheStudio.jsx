...
export default function FicheStudio({
    ...
    const toggleSlide = (slideNumber) => {
        ...
        setFormData((prev) => {
            ...
            return {
                ...prev,
                selectedSlides: [...current].sort((a, b) => a - b)
            };
        });
    };
    ...
    const previewSlides = slidesManifest.filter((slide) => selectedSlides.has(Number(slide?.slideNumber || 0)));
            ...
            <div className="expose-studio-body">
                <div className="expose-editor-card">
                    ...
                    <textarea
                        ...
                        placeholder="Ex: réalise une fiche de révision claire avec définitions, dates, mots-clés et au moins une image."
                    />
                    ...
                    <div className="rounded-2xl border border-slate-200 bg-slate-50 p-3">
                        ...
                        {!slidesLoading && !slidesError && slidesManifest.length === 0 && (
                            <div className="text-[11px] font-bold text-slate-400">Colle un lien Google Slides pour charger les slides.</div>
                        )}
                                ...
                                {slidesManifest.map((slide) => {
                                    ...
                                    return (
                                        <button
                                            ...
                                            className={`rounded-2xl border p-2 text-left transition-all ${active ? 'border-indigo-500 bg-indigo-50 shadow-md' : 'border-slate-200 bg-white'}`}
                                        >
                    ...
                    <div className="text-[11px] font-black uppercase text-slate-400 mt-4 mb-2">Fiches rendues</div>
                    <div className="max-h-[280px] overflow-auto rounded-xl border border-slate-200 bg-slate-50 p-2 space-y-2">
                        {expectedStudentIds.length === 0 && submissions.length === 0 && (
                            <div className="text-[11px] font-bold text-slate-400">Aucune fiche pour le moment.</div>
                        )}
                                    ...
                                    <div className="text-[12px] font-black text-slate-700 flex items-center justify-between gap-2">
                                        ...
                                        {done && (
                                            <button
                                                type="button"
                                                onClick={async () => {
                                                    const nextValue = !(sub?.teacherValidated === true);
                                                    const res = await fetch(`/api/fiches/${encodeURIComponent(String(formData._id || ''))}/submissions/${encodeURIComponent(resolvedId)}/validate`, {
                                                        method: 'PATCH',
                                                        headers: { 'Content-Type': 'application/json' },
                                                        body: JSON.stringify({ teacherValidated: nextValue })
                                                    });
                                                    if (!res.ok) return alert('Validation impossible.');
                                                    const data = await res.json().catch(() => ({}));
                                                    setFormData((prev) => ({
                                                        ...prev,
                                                        submissions: (prev.submissions || []).map((row) => (
                                                            String(row?.studentId || '') === resolvedId ? { ...row, ...(data?.submission || {}), studentId: resolvedId } : row
                                                        ))
                                                    }));
                                                }}
                                                className={`text-[10px] font-black px-2 py-1 rounded-lg border ${sub?.teacherValidated === true ? 'bg-emerald-100 text-emerald-700 border-emerald-200' : 'bg-slate-100 text-slate-700 border-slate-200'}`}
                                            >
                                                {sub?.teacherValidated === true ? 'VALIDÉE' : 'VALIDER'}
                                            </button>
                                        )}
                                    </div>
                                    <div className="text-[11px] font-bold mt-1">
                                        {done ? '✅ Fiche déposée' : '⏳ En attente'}
                                    </div>
                                    <div className="text-[10px] font-bold text-slate-500 mt-1">
                                        {done ? `${Math.max(0, Number(sub?.plainText?.length || 0))} caractères • ${Number(sub?.imageCount || 0)} image(s)` : 'Aucun contenu enregistré.'}
                                    </div>
                                    {done && (
                                        <div
                                            className="mt-2 rounded-xl border border-slate-200 bg-slate-50 p-2 max-h-[180px] overflow-auto text-[12px] text-slate-700"
                                            dangerouslySetInnerHTML={{ __html: String(sub?.contentHtml || '') }}
                                        />
                                    )}
                                </div>
                            );
                ...
                <StudioDistributionSidebar
                    ...
                    loading={loading}
                    onSave={handleSave}
                    saveLabel={loading ? 'SAUVEGARDE...' : 'ENREGISTRER FICHE'}
                />
            </div>
        </div>
    );
