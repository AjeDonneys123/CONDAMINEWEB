...
export default function HomeworkStudio({ initialData, chapters, globalClass, onClose }) {
  ...
  const handleUpload = async (files, idx, type) => {
    ...
    if (!formData.title.trim()) {
        alert("⚠️ Donne un TITRE au devoir avant d'ajouter des fichiers (nécessaire pour créer le dossier Drive).");
        return;
    }
    ...
    for (let file of Array.from(files)) {
      ...
      try {
        const response = await fetch('/api/homework/upload-to-drive', { method: 'POST', body: fd });
        const data = await response.json();
        
        if (data.ok) {
          if (type === 'doc') newLevels[idx].attachmentUrls.push(data.imageUrl);
          else newLevels[idx].questionImage = data.imageUrl;
        } else {
            throw new Error(data.error);
        }
      } catch (e) { 
          console.error("Upload crash:", e);
          alert("Erreur upload Drive : " + e.message);
      }
    }
    ...
    <div className="homework-studio-overlay animate-in slide-in-from-bottom-4">
        <div className="p-6 bg-orange-500 text-white flex justify-between items-center shadow-lg pt-12">
            <div className="flex flex-col">
                ...
                <span className="text-[10px] font-black opacity-80 uppercase tracking-widest">Aiguillage Drive : {globalClass}</span>
            </div>
        ...
        <div className="flex-1 overflow-y-auto p-8 custom-scrollbar">
            <div className="max-w-5xl mx-auto space-y-8">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className="assign-card"><label className="text-[10px] font-black text-slate-400 uppercase mb-2 block">Dossier Chapitre</label>
                    ...
                    <div className="assign-card relative"><label className="text-[10px] font-black text-slate-400 uppercase mb-2 block">Cibler des élèves</label>
                        <input className="w-full outline-none bg-white p-3 rounded-xl border border-slate-200 font-bold" placeholder="Chercher..." value={stdSearch} onChange={e=>setStdSearch(e.target.value)} />
                        {stdSearch && (
                ...
                {formData.levels.map((lvl, idx) => (
                    <div key={idx} className="bg-white p-10 rounded-[50px] border border-slate-100 shadow-sm relative">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                            <div>
                                <label className="text-[10px] font-black uppercase text-blue-500 mb-2 block">Documents Drive</label>
                            ...
                            <div>
                                <label className="text-[10px] font-black uppercase text-orange-500 mb-2 block">Image Question</label>
                        ...
                        <div className="mt-8">
                            <textarea className="w-full p-6 rounded-3xl h-24 border border-slate-200 font-bold text-slate-600" value={lvl.instruction} onChange={e=>{const n=[...formData.levels]; n[idx].instruction=e.target.value; setFormData({...formData, levels:n});}} placeholder="Consigne..." />
                        </div>
                    </div>
                ))}
            </div>
        ...
        <div className="p-8 bg-white border-t">
            <button onClick={save} disabled={uploading} className="w-full p-6 bg-orange-500 text-white font-black text-2xl rounded-3xl shadow-xl hover:bg-orange-600 transition-all uppercase">{uploading ? "SYNC DRIVE & DISTRIBUTION..." : "💾 SAUVEGARDER ET DISTRIBUER"}</button>
        </div>
    </div>
  );
}