    ...
    <div className="homework-studio-overlay animate-in slide-in-from-bottom-4">
        {saveStatus.active && (
            <div className="fixed inset-0 z-[7000] flex items-center justify-center p-6 bg-slate-900/80 backdrop-blur-xl">
                <div className={`bg-white rounded-[40px] p-10 max-w-2xl w-full text-center shadow-2xl border-4 ${saveStatus.driveError ? 'border-red-500' : 'border-emerald-500'} animate-in zoom-in`}>
                    ...
                    <h2 className="text-2xl font-black text-slate-800 uppercase mb-2">{saveStatus.driveError ? "Erreur de Synchro" : saveStatus.message}</h2>
                    <div className="bg-slate-50 p-6 rounded-3xl border-2 border-dashed border-slate-200 text-left my-6">
                        <span className="text-[10px] font-black uppercase text-slate-400 block mb-2">Emplacement vérifié sur Drive :</span>
                        <code className="text-xs font-mono font-bold text-slate-600 break-all leading-relaxed">{saveStatus.path}</code>
                    </div>
                    <button onClick={onClose} className="w-full p-5 bg-slate-900 text-white font-black rounded-2xl uppercase tracking-widest">Fermer le Studio</button>
                </div>
            </div>
        )}
        <div className="p-6 bg-orange-500 text-white flex justify-between items-center shadow-lg pt-12">
            <div className="flex flex-col">
                ...
                <span className="text-[10px] font-black opacity-80 uppercase tracking-widest">PROF : {user.firstName} {user.lastName} | CLASSE : {globalClass}</span>
            </div>
