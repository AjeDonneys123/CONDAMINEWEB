...
export default function HomeworkStudio({ initialData, chapters, globalClass, onClose }) {
  ...
  const handleUpload = async (files, idx, type) => {
    ...
    if (!formData.title) return alert("Donne d'abord un titre au devoir pour créer le dossier Drive.");
    ...
    for (let file of Array.from(files)) {
      ...
      try {
        // APPEL AU NOUVEAU SERVICE DRIVE
    ...
    <div className="homework-studio-overlay animate-in slide-in-from-bottom-4">
        <div className="p-6 bg-orange-500 text-white flex justify-between items-center shadow-lg pt-12">
            <div className="flex flex-col">
                ...
                <span className="text-[10px] font-black opacity-60 uppercase tracking-tighter">Stockage : Google Drive (CONDACLASSE / {globalClass})</span>
            </div>
                        ...
                        {stdSearch && (
                            <div className="absolute top-full left-0 right-0 bg-white shadow-xl z-50 rounded-xl max-h-40 overflow-auto border">
                                {filteredPlayers.map(p => (
                                    <div key={p._id} onClick={()=>{ setFormData({...formData, targetPlayerIds:[...formData.targetPlayerIds, p._id]}); setStdSearch(''); }} className="p-3 hover:bg-orange-50 cursor-pointer font-bold border-b">{p.firstName} {p.lastName}</div>
                                ))}
                            </div>
                        )}
        ...
        <div className="p-8 bg-white border-t">
            <button onClick={save} disabled={uploading} className="w-full p-6 bg-orange-500 text-white font-black text-2xl rounded-3xl shadow-xl hover:bg-orange-600 transition-all uppercase">{uploading ? "SÉCURISATION DRIVE..." : "💾 SAUVEGARDER LE DEVOIR"}</button>
        </div>
    </div>
  );
}