...
export default function SoundEditorModal({ soundUrl, soundName, onSave, onClose, resolveUrl }) {
    // ... (Code identique, juste l'import modifié) ...
    ...
    return (
        <div className="se-modal-overlay">
            <div className="se-window animate-in zoom-in">
                {isProcessing && (<div className="absolute inset-0 bg-white/80 z-50 flex items-center justify-center flex-col"><div className="text-4xl animate-spin">⚙️</div><span className="font-black text-purple-600 mt-2 uppercase">Traitement...</span></div>)}
                <div className="se-header">
                    <input className="se-title-input" value={name} onChange={e => setName(e.target.value)} />
                    <div className="se-header-actions"><button className="se-btn-main btn-undo" onClick={handleUndo} disabled={history.length <= 1}>↩ Undo</button><button className="se-btn-main btn-save" onClick={handleSaveInternal}>💾 Sauver</button><button className="se-btn-main btn-close" onClick={onClose}>✕</button></div>
                </div>
                <div className="se-wave-area relative">
                    <canvas ref={canvasRef} className="se-canvas" onMouseDown={handleMouseDown} />
                    <div className="absolute top-2 left-2 bg-slate-900/80 text-white p-2 rounded text-[10px] font-mono pointer-events-none z-50"><strong>DIAGNOSTIC:</strong><br/>{debugInfo}</div>
                ...
                <div className="se-toolbar">
                    ...
                    {(trimStart > 0 || trimEnd < 1) && (<button className="se-tool-btn" onClick={() => applyEffect('TRIM')}><div className="se-tool-icon text-red-500 border-red-200">✂️</div><span className="se-tool-label text-red-500">Couper</span></button>)}
