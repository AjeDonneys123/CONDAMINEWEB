// @signatures: UnifiedMoteur, handleBridgeEvent, handleAnswerClick, handleRoundValidation, pickNextQuestion
...
/**
 * 🧠 UNIFIED MOTEUR V4.6 (BRIDGE + UI RESTORED)
 * - Architecture : Bridge (Signal JS -> React)
 * - UI : Intro, Barres, Vies restaurés.
 */
export default function UnifiedMoteur({ gameData, onExit, isStudioTest = false }) {
    // --- ÉTATS ---
    const [lives, setLives] = useState(4);
    const [questionStates, setQuestionStates] = useState([]); // [0, 1, 2, 3...]
    ...
    // --- UI STATES ---
    const [showLevelIntro, setShowLevelIntro] = useState(true); // Restauré à TRUE par défaut
    ...
    const [zoomMedia, setZoomMedia] = useState(null); // Pour la fiche/vidéo plein écran
    ...
    const handleBridgeEvent = useCallback((type, value) => {
        switch(type) {
            ...
            case 'AUDIO': /* triggerGlobalEvent(value) - A implémenter si besoin */; break;
    ...
    // Initialisation Niveaux & Questions
    useEffect(() => {
        const initLevel = async () => {
            let levelsData = gameData?.levels || [];
            
            // Si pas de niveaux (ex: Test Studio), on charge le mock
            ...
            if (levelsData[currentLevelIdx]) {
                ...
                // RÉPARATION : On force l'init des barres ici
                setQuestionStates(new Array(qs.length).fill(0)); 
                setCurrentQIndex(0);
                setShowLevelIntro(true); // On force l'écran d'intro au chargement
            }
        };
        initLevel();
    ...
    // Assets Loading
    useEffect(() => {
        if (!assetsSignature) { setIsReady(true); return; } // Si pas d'assets, on est prêt direct
    ...
    const handleAnswerClick = (val) => {
        ...
        // On envoie au JS pour l'animation
    ...
    const triggerWinSequence = () => { 
        ...
        setTimeout(() => { 
            ...
            if(allLevels[currentLevelIdx+1]) {
                ...
                setEngineStarted(false); // Stop render pour afficher intro suivante
    ...
    }, [engineStarted, handleBridgeEvent]);
    ...
    return (
        <div className={`fixed inset-0 z-[99999] bg-slate-950 flex flex-col items-center justify-center overflow-hidden font-sans ${isShake ? 'animate-shake' : ''}`}>
            {/* DEBUGGER */}
            <div className="absolute top-2 left-4 px-3 py-1 bg-black/50 text-[10px] font-black text-yellow-500 rounded-full border border-yellow-500/30 z-[5000]">MOTEUR V4.6 (UI FIXED)</div>
                ...
                <div className="absolute inset-0 flex flex-col items-center justify-center bg-slate-950 z-[6000] animate-in zoom-in p-8 text-center">
                    ...
                    <div className="flex gap-10 mb-12 w-full max-w-4xl justify-center h-[280px]">
                        ...
                        <div onClick={() => currentLevelData.intro?.sheetUrl && setZoomMedia('sheet')} className="h-full aspect-[3/4] bg-slate-800 rounded-3xl border-4 border-slate-700 overflow-hidden flex items-center justify-center shadow-2xl cursor-pointer hover:border-indigo-500 hover:scale-105 transition-all relative group">
                            ...
                            ) : <span className="text-slate-500 font-bold">PAS DE FICHE</span>}
                            <div className="absolute bottom-4 left-0 right-0 text-center opacity-0 group-hover:opacity-100 transition-opacity bg-black/50 text-white text-xs font-bold py-1">AGRANDIR</div>
                        </div>
                        ...
                        <div onClick={() => currentLevelData.intro?.videoUrl && setZoomMedia('video')} className="h-full aspect-video bg-black rounded-3xl border-4 border-slate-700 overflow-hidden shadow-2xl flex items-center justify-center cursor-pointer hover:border-indigo-500 hover:scale-105 transition-all relative group">
                            ...
                            ) : <span className="text-slate-500 font-bold">PAS DE VIDÉO</span>}
                            <div className="absolute bottom-4 left-0 right-0 text-center opacity-0 group-hover:opacity-100 transition-opacity bg-black/50 text-white text-xs font-bold py-1">LIRE</div>
                        </div>
                    </div>

                    <button onClick={startCurrentLevel} disabled={!isReady} className={`px-16 py-6 rounded-full font-black text-3xl shadow-2xl border-4 transition-all ${isReady ? 'bg-white text-indigo-900 border-indigo-500 hover:scale-110' : 'bg-slate-800 text-slate-500 cursor-not-allowed'}`}>
            ...
            {engineStarted && !showLevelIntro && (
                <>
                    {/* HUD BARRES RESTAURÉES */}
                    <div className="absolute top-6 w-full flex justify-between px-10 pointer-events-none z-30">
                        <div className="bg-slate-900/80 p-3 px-6 rounded-2xl border-2 border-slate-700 text-3xl shadow-lg">{"❤️".repeat(lives)}</div>
                        
                        {activeBossVisual && <div className="text-red-500 font-black text-3xl animate-pulse drop-shadow-lg border-b-4 border-red-600">⚠️ BOSS ⚠️</div>}
                        ...
                        <div className="absolute bottom-10 w-full flex justify-center px-10 pointer-events-auto z-30">
                            <div className="grid grid-cols-4 gap-4 w-full max-w-5xl">
                                {safeQ.options.map((o, i) => (
                                    <button key={i} onClick={() => handleAnswerClick(i)} className="bg-indigo-600 text-white py-6 rounded-2xl font-black uppercase text-lg border-b-8 border-indigo-800 active:border-b-0 active:translate-y-2 hover:bg-indigo-500 transition-colors shadow-lg">
                                        {o}
                                    </button>
                                ))}
                            </div>
