// @signatures: UnifiedMoteur, handleBridgeEvent, handleAnswerClick, handleRoundValidation, pickNextQuestion, handleHeartClick, handleQuestionClick, handleBarClick
...
/**
 * 🧠 UNIFIED MOTEUR V4.7 (FULL UI + CHEATS)
 * - Questions : Réintégrées au centre.
 * - Cheats : Réactivés (Touche 'S' + Clic).
 */
export default function UnifiedMoteur({ gameData, onExit, isStudioTest = false }) {
        ...
        setQuestionStates(prev => {
            ...
            setTimeout(() => {
                ...
                if (available.length > 0) {
                    if (isCorrect && next[currentQIndex] === 2) { handleBridgeEvent('SET_BOSS', true); } 
                    else {
    ...
    const triggerWinSequence = () => { 
        ...
        setTimeout(() => { 
            ...
            if(allLevels[currentLevelIdx+1]) { setCurrentLevelIdx(p=>p+1); setEngineStarted(false); setShowLevelIntro(true); } 
            else { setShowGameComplete(true); }
        }, 3000); 
    };

    // --- CHEAT CODES (Touche S) ---
    const isCheatMode = () => keysPressed.current['KeyS'];
    ...
    const handleBarClick = (idx) => { 
        if (isCheatMode()) setQuestionStates(prev => { const n = [...prev]; n[idx] = Math.min(3, n[idx] + 1); return n; }); 
    };
    ...
    useEffect(() => {
        ...
        try {
            const MiniGameBase = createGameBase({ 
                audioBuffers: audioBuffersRef.current, audioCtx: audioCtxRef.current, projectRef, sceneIdx: 0, imageAssets: imageAssetsRef.current, resolveUrl, canvas: canvasRef.current, ctx: canvasRef.current.getContext('2d'), playParallelSound: () => {}, 
                bridge: { trigger: handleBridgeEvent },
                callbacks: { onRoundEnd: handleRoundValidation } 
            });
    ...
    return (
        <div className={`fixed inset-0 z-[99999] bg-slate-950 flex flex-col items-center justify-center overflow-hidden font-sans ${isShake ? 'animate-shake' : ''}`}>
            {/* DEBUGGER */}
            <div className="absolute top-2 left-4 px-3 py-1 bg-black/50 text-[10px] font-black text-yellow-500 rounded-full border border-yellow-500/30 z-[5000]">MOTEUR V4.7 (FULL UI)</div>
            ...
            {showLevelIntro && (
                <div className="absolute inset-0 flex flex-col items-center justify-center bg-slate-950 z-[6000] animate-in zoom-in p-8 text-center">
                    ...
                    <button onClick={startCurrentLevel} disabled={!isReady} className="px-16 py-6 rounded-full font-black text-3xl shadow-2xl border-4 transition-all bg-white text-indigo-900 border-indigo-500 hover:scale-110">{isReady ? "JOUER 🚀" : `CHARGEMENT ${loadProgress}...`}</button>
                </div>
            )}
            ...
            {engineStarted && !showLevelIntro && (
                <>
                    ...
                    <div className="absolute top-6 w-full flex justify-between px-10 pointer-events-none z-30">
                        {/* CŒURS */}
                        <div className="bg-slate-900/80 p-3 px-6 rounded-2xl border-2 border-slate-700 text-3xl shadow-lg pointer-events-auto cursor-pointer" onClick={handleHeartClick}>{"❤️".repeat(lives)}</div>
                        
                        {/* 🚀 BOITE QUESTION (RESTAURÉE) */}
                        <div className="flex-1 flex flex-col items-center gap-2 mx-10">
                            <div className="bg-indigo-600 text-white px-4 py-1 rounded-full text-xs font-black uppercase tracking-widest">{currentLevelData.name}</div>
                            {safeQ && (
                        ...
                        <div className="flex gap-2 pointer-events-auto">
                            {questionStates.map((s, i) => (
                                <div key={i} onClick={() => handleBarClick(i)} className={`w-6 h-16 rounded-lg border-2 ${currentQIndex === i ? 'border-white scale-110 shadow-lg' : 'border-slate-600 opacity-60'} bg-slate-800 overflow-hidden relative transition-all cursor-pointer`}>
