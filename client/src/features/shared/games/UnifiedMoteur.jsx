...
/**
 * 🧠 UNIFIED MOTEUR V8.3 (THE PLATFORMER UPDATE)
 * Rôle : Coque universelle supportant les interactions par collision.
 * Nouveauté : Gère SUBMIT_ANSWER venant du script (Mario style).
 */
...
export default function UnifiedMoteur({ gameData, onExit, isStudioTest = false }) {
    ...
    const bridgeProxy = useRef((type, value) => {
        switch(type) {
            ...
            case 'SUBMIT_ANSWER': // ✅ NOUVEAU POUR MARIO STYLE
    ...
    const handleAnswerClick = (val) => {
        ...
        // ✅ GESTION MIXTE : Index (Mario) ou Valeur (Bouton/Boss)
    ...
    useEffect(() => {
        ...
        try {
            const MiniGameBase = createGameBase({ 
                ...
                // ✅ PASSAGE DES QUESTIONS AU SCRIPT
            ...
            const tick = () => {
                if (instance && !instance.isStopped) {
                    ...
                    // ✅ MISE A JOUR CONSTANTE DE L'INDEX DE QUESTION
    ...
    return (
        <div className={"fixed inset-0 z-[99999] bg-slate-950 flex flex-col items-center justify-center overflow-hidden font-sans " + (isShake ? 'animate-shake' : '')}>
            <div className="absolute top-2 left-4 px-3 py-1 bg-black/50 text-[10px] font-black text-yellow-500 rounded-full border border-yellow-500/30 z-[5000]">MOTEUR V8.3 (PLATFORMER ENGINE)</div>
