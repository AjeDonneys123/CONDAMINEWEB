// @signatures: UnifiedMoteur, handleAnswerClick, triggerWinSequence, startCurrentLevel, handleRetry, performDeepReset, isCheatActive, handleBarClick, handleQuestionClick, handleHeartClick
...
/**
 * 🧠 UNIFIED MOTEUR V8.5 (FULL RECOVERY)
 * Restaure : Cheatcodes (clics HUD), Sons de départ, et stabilité Tapping.
 */
...
export default function UnifiedMoteur({ gameData, onExit, isStudioTest = false }) {
    ...
    useEffect(() => {
        const initLevel = async () => {
            ...
            if (isStudioTest || levelsData.length === 0) { try { const res = await api.get('/games/test-data'); levelsData = res?.levels?.length > 0 ? res.levels : []; } catch(e) {} }
    ...
    const performDeepReset = () => {
        setLives(4); setActiveBossVisual(false); bossModeRef.current = false; setShowAnswerUI(true);
    ...
    return (
        <div className={"fixed inset-0 z-[99999] bg-slate-950 flex flex-col items-center justify-center " + (isShake ? 'animate-shake' : '')}>
            <div className="absolute top-2 left-4 px-3 py-1 bg-black/50 text-[10px] font-black text-yellow-500 rounded-full border border-yellow-500/30 z-[5000]">MOTEUR V8.5</div>
            ...
            {engineStarted && !showLevelIntro && (
                <>
                    <div className="absolute top-6 w-full flex justify-between px-10 pointer-events-none z-30">
                        ...
                        <div className="flex-1 mx-10 pointer-events-auto cursor-pointer">
                            <div onClick={handleQuestionClick} className={"bg-slate-900/95 text-white font-black py-4 px-10 rounded-2xl border-2 text-xl text-center border-slate-600 " + (activeBossVisual ? 'border-red-500 ring-2 ring-red-500/50' : '')}>
