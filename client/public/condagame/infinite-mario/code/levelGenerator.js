...
Mario.LevelGenerator.prototype = {
    CreateLevel: function(type, difficulty) {
        ...
        // Create exactly one reachable gem block for every quiz question in this run.
        level.QuizBoxes = {};
        var quizCount = Math.max(0, Math.floor(Number(Mario.LevelGemGoal) || 0));
        var quizIndex = 0;
        var minQuizX = 12;
        var maxQuizX = level.Width - 11;
        for (quizIndex = 0; quizIndex < quizCount && minQuizX <= maxQuizX; quizIndex++) {
            var preferredX = Math.round(minQuizX + (maxQuizX - minQuizX) * (quizIndex + 1) / (quizCount + 1));
            var radius = 0;
            var placed = false;
            for (radius = 0; radius < level.Width && !placed; radius++) {
                var offsets = radius === 0 ? [0] : [-radius, radius];
                for (var offsetIndex = 0; offsetIndex < offsets.length && !placed; offsetIndex++) {
                    x = preferredX + offsets[offsetIndex];
                    if (x < minQuizX || x > maxQuizX) continue;
                    var groundY = this.Height - 1;
                    while (groundY > 2 && level.GetBlock(x, groundY) === 0) groundY--;
                    y = groundY - 4;
                    var quizKey = `${x},${y}`;
                    if (y > 1 && level.GetBlock(x, y) === 0 && level.GetBlock(x, y - 1) === 0 && !level.QuizBoxes[quizKey]) {
                        level.SetBlock(x, y, 18);
                        level.QuizBoxes[quizKey] = true;
                        placed = true;
                    }
                }
            }
