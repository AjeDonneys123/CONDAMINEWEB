...
class MiniGame extends MiniGameBase {
    ...
    onResult(isCorrect) {
        ...
        if (isCorrect) {
            ...
            if (this.HEROS) {
                this.HEROS.scale = this.bossScale; // Devient plus grand sans dépasser les plateformes
    ...
    checkAnswer(idx) {
        ...
        if (idx === correct) {
            ...
            this.velocity = Math.max(this.velocity, 2.2); // Force la chute et traverse les plateformes
            setTimeout(() => { if (!this.isStopped) this.resetInternalState(); }, 700);
        }
    }
